...
class MEC_feature_profile extends MEC_base
{
    ...
    public function profile($atts = array())
    {
        ...
        {
            // Show message
            $message = sprintf(__('Please %s/%s in order to see your bookings / profile.', 'mec'), '<a href="'.wp_login_url($this->main->get_full_url()).'">'.__('Login', 'mec').'</a>', '<a href="'.wp_registration_url().'">'.__('Register', 'mec').'</a>');
