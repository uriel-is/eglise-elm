    ...
    foreach ($labels as $key => $label) {
        switch ($label) {
            ...
            case 'WE':
                ...
                break;
            case 'TH':
                $labels[$key] = esc_html__('THU', 'mec-fl');
                break;
            case 'FR':
        ...
        if (isset($events[$today]) and count($events[$today])) {
            ...
            foreach ($events[$today] as $event) {
                ...
                if ($eventItems >= 2) {
                    ...
                    echo '<h4 class="mec-event-title"><a data-event-id="'.$event->data->ID.'" style="color:' . $event_color . ';" class="event-single-link-simple" href="'.$this->main->get_event_date_permalink($event, $event->date['start']['date']).'">'.$event->data->title.'</a></h4>';
                    echo '</div>';
                }
