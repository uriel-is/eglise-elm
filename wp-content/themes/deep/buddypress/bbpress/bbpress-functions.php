...
class BBP_Default extends BBP_Theme_Compat {
	...
	private function setup_globals() {
		$bbp           = bbpress();
		$this->id      = 'default';
		$this->name    = esc_html__( 'bbPress Default', 'deep' );
	...
	public function enqueue_styles() {
		...
		$file = DEEP_ASSETS_URL . 'css/frontend/bbpress/bbpress.css';
		...
		wp_enqueue_style( $handle, $file, array(), $this->version, 'screen' );
	}
	...
	public function localize_topic_script() {
		...
		wp_localize_script( 'bbpress-topic', 'bbpTopicJS', array(
			...
			'generic_ajax_error' => esc_html__( 'Something went wrong. Refresh your browser and try again.', 'deep' ),
	...
	public function ajax_favorite() {
		...
		if ( ! bbp_is_favorites_active() ) {
			bbp_ajax_response( false, esc_html__( 'Favorites are no longer active.', 'deep' ), 300 );
		}

		// Bail if user is not logged in
		if ( !is_user_logged_in() ) {
			bbp_ajax_response( false, esc_html__( 'Please login to make this topic a favorite.', 'deep' ), 301 );
		}
		...
		if ( !current_user_can( 'edit_user', $user_id ) ) {
			bbp_ajax_response( false, esc_html__( 'You do not have permission to do this.', 'deep' ), 302 );
		}
		...
		if ( empty( $topic ) ) {
			bbp_ajax_response( false, esc_html__( 'Topic not Found.', 'deep' ), 303 );
		}

		// Bail if user did not take this action
		if ( !isset( $_POST['nonce'] ) || !wp_verify_nonce( $_POST['nonce'], 'toggle-favorite_' . $topic->ID ) ) {
			bbp_ajax_response( false, esc_html__( 'Are you sure you meant to do that?', 'deep' ), 304 );
		}
		...
		if ( empty( $status ) ) {
			bbp_ajax_response( false, esc_html__( 'Request unsuccessful. Please try again.', 'deep' ), 305 );
		}
	...
	public function ajax_subscription() {
		...
		if ( !bbp_is_subscriptions_active() ) {
			bbp_ajax_response( false, esc_html__( 'Subscriptions are no longer active.', 'deep' ), 300 );
		}

		// Bail if user is not logged in
		if ( !is_user_logged_in() ) {
			bbp_ajax_response( false, esc_html__( 'Please login to subscribe to this topic.', 'deep' ), 301 );
		}
...
/**
 * Remove topic & reply redeep log
 * -------------------------------------------------------------------------------------------*/
remove_filter( 'bbp_get_reply_content', 'bbp_reply_content_append_redeeps',  1,  2 );
remove_filter( 'bbp_get_topic_content', 'bbp_topic_content_append_redeeps',  1,  2 );
