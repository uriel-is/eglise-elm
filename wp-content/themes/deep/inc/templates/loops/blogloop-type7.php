		...
		if( $deep_options['deep_blog_featuredimage_enable'] ) {
			if( 'video'  == $post_format || 'audio'  == $post_format ) {
				...
				if( has_post_thumbnail() ){
					echo '<a href="' . get_the_permalink() . '"> '.wp_get_attachment_image( get_post_thumbnail_id(), array( '548', '394' ) ).' </a>';
				} else {
		...
		<div class="blgtyp1-contnet">
			<div class="post-meta-data">
				...
				if($deep_options['deep_blog_meta_category_enable']){ ?>
					<div class="blog-cat">
						<span class="category-color" style="background: <?php echo deep_category_color(); ?>;"></span>
						<?php the_category(', ') ?>
					</div>
				<?php } ?>
				...
				<?php if ( $deep_options['deep_blog_meta_date_enable'] ) { ?>
					...
					<div class="blog-date">
						<a href="<?php echo get_the_permalink(); ?>"><?php the_time(get_option( 'date_format' )) ?></a>
					</div>
