							...
							<div class="gl-img owl-carousel owl-theme">
								...
								for( $i=0; $i < sizeof( $galley_id ); $i++ ) {
									if( !empty( $galley_id[$i] ) ) {
										...
										echo '<img src="' . esc_url( $thumbnail_url ) . '" alt="' . get_post_meta( $galley_id[$i], '_wp_attachment_image_alt', true ) . '">';
									}
								} ?>			
							</div>
						</div>
				...
				<div class="blgtyp5-contnet">
					<div class="post-meta-data">
						...
						if( $deep_options['deep_blog_meta_category_enable'] ) { ?>
							<div class="blog-cat"> <span class="category-color" style="background: <?php echo deep_category_color(); ?>;"></span> <?php the_category(', ') ?> </div>
							<?php
						}
						if ( $deep_options['deep_blog_meta_date_enable'] ) { ?>
							<h6 class="blog-date"> <a href="<?php echo get_the_permalink(); ?>"><?php the_time( get_option( 'date_format' ) ) ?></a> </h6>
							<?php
						} ?>
					...
					if( $deep_options['deep_blog_posttitle_enable'] && $post_format !='aside' && $post_format !='quote') { 	
						if( 'link' == $post_format ) {
							...
							<h3 class="post-title"><a class="hcolorf" href="<?php echo esc_url($link); ?>"><?php the_title() ?></a></h3>
							<?php
						} else { ?>
					...
					echo '<blockquote>';
					echo '<p>' . deep_excerpt( ( $deep_options['deep_blog_excerpt_list'] ) ? $deep_options['deep_blog_excerpt_list'] : 17 ) . '</p>';
					if( $post_format == 'quote')
					echo '</blockquote>';
					if( $post_format == ('quote') || $post_format == 'aside' )
					echo '<a class="readmore" href="'. get_permalink( get_the_ID() ) . '">' . esc_html__('View Post', 'deep') . '</a>';
					?>	
				</div>
			</div>
</article>