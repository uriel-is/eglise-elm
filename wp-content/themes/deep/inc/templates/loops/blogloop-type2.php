							...
							<div class="gl-img owl-carousel owl-theme">
								...
								for( $i=0; $i < sizeof( $galley_id ); $i++ ) {
									if( !empty( $galley_id[$i] ) ) {
										...
										echo '<img src="' . esc_url( $thumbnail_url ) . '" ' . get_post_meta( $galley_id[$i], '_wp_attachment_image_alt', true ) . '>';
									}
								} ?>			
							</div>
						</div>
...
<?php } ?>
		...
		echo '<a class="readmore" href="' . get_permalink($post->ID) . '">' . esc_html($deep_options['deep_blog_readmore_text']) . '</a>';
		if($post_format == 'quote') echo '</blockquote>';
