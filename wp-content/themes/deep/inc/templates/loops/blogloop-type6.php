		...
		<div class="latest-b21-cont">
			...
			if( $deep_options['deep_blog_meta_date_enable'] ) { ?>
				<div class="latest-b21-date">
					<i class="pe-7s-clock"></i><a href="<?php echo get_the_permalink(); ?>"><?php echo get_the_date();?></a>
				</div>
			<?php }
			...
			if( $deep_options['deep_blog_posttitle_enable'] ) {
				if( ('aside' != $post_format ) && ('quote' != $post_format) ) {
					if( 'link' == $post_format ) {
						...
						<h3 class="latest-b21-title"><a href="<?php echo esc_url($link); ?>"><?php the_title() ?></a></h3> <?php
					} else {
						?>
