...
class Michigan_Course_Categories extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			esc_attr__( 'Webnus Course Categories', 'deep' ), // Name
			array( 'description' => esc_attr__( 'A Course Categories Widget', 'deep' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( ! empty( $categories ) && ! is_wp_error( $categories ) ) :
			$all_categoy = $category_icon ? '<i class="fa fa-book"></i>': '';
			$post_type = get_post_type_object(get_post_type());
			echo '<ul class="course-categories list">';
					echo'
						<li class="course-category">
							<a class="hcolorf" href="' . esc_url( get_site_url() ) . '/'.$w_courses_name.'/" title="' . esc_attr( 'View all Categories', 'deep' ). '">'.$all_categoy . esc_html__('All Courses','deep'). '</a>
						</li>';
				...
				foreach ( $categories as $category ) :
							...
							foreach ( $child_categories as $child_category ) :
								...
								$childs .= '
									<li class="course-category course-category-child">
										<a class="hcolorf" href="' . esc_url( get_category_link( $child_category ) ) . '" title="' . esc_attr( sprintf( __( 'View all courses under %s', 'deep' ), $child_category->name ) ) . '">'. $cat_icon . esc_html( $child_category->name ) .'<span>'.$count.'</span>' . '</a>
									</li>';
							endforeach;
						$childs .= '</ul>';
					endif;
					...
					echo '
						<li class="course-category' . $parent_class . '">
							<a class="hcolorf" href="' . esc_url( get_category_link( $category ) ) . '" title="' . esc_attr( sprintf( __( 'View all courses under %s', 'deep' ), $category->name ) ) . '">'. $cat_icon . esc_html( $category->name ) .'<span>'.$count.'</span>' . '</a>
							' . $childs . '
						</li>';
				endforeach;
	...
	public function form( $instance ) {
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'post_counts' ); ?>"><?php esc_html_e( 'Show post counts', 'deep' ); ?></label> 
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'category_icon' ); ?>"><?php esc_html_e( 'Show Category Icon', 'deep' ); ?></label> 
		</p>
