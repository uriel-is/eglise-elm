...
class Michigan_More_Course extends WP_Widget {
	function __construct() {
		parent::__construct(
			...
			esc_attr__( 'Webnus More Course', 'deep' ), // Name
			array( 'description' => esc_attr__( 'A more course widget for your site', 'deep' ), ) // Args
		);
	}
	public function widget( $args, $instance ) {
		...
		if($type) {
			switch($type){
				case '1':
				$orderby = 'date';
				$maintitle = esc_html__('Recent Courses','deep');
				break;
				case '2':
				$orderby = 'meta_value_num';
				$maintitle = esc_html__('Popular Courses','deep');
				break;
				case '3':
				$orderby = 'rate';
				$maintitle = esc_html__('Top Rated Courses','deep');
				break;
				case '4':
				$orderby = 'rand';
				$maintitle = esc_html__('More Courses','deep');
				break;
			}
			GLOBAL $post;
			$post_ids = array();
		...
		echo '<h4 class="subtitle">'.$maintitle.'</h4><div id="more-courses" class="w-crsl owl-carousel owl-theme">';
			...
			<article class="modern-grid llms-course-list"><div class="llms-course-link">
				...
				<div class="modern-content">
					...
					<div class="clearfix modern-meta">
						...
						echo ($course_students)?'<span class="modern-students" title="'.esc_attr('Enrolled Students','deep').'"><i class="sl-people"></i>'.$course_students.'</span>':'<span class="modern-viewers" title="'.esc_attr('Viewers','deep').'"><i class="fa-eye"></i>'.deep_getViews(get_the_ID()).'</span>';
						echo '</div>';
						?>
					</div>
				</div>
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('type'); ?>"><?php esc_html_e( 'More Courses Type:', 'deep' ); ?></label>
			<select id="<?php echo $this->get_field_id('type'); ?>" name="<?php echo $this->get_field_name('type'); ?>" class="widefat type" style="width: 100%;">
				<option value="<?php echo esc_attr( '1' ); ?>" <?php if ( $type == '1' ) echo 'selected="selected"'; ?>><?php esc_html_e('Recent Courses', 'deep'); ?></option>
				<option value="<?php echo esc_attr( '2' ); ?>" <?php if ( $type == '2' ) echo 'selected="selected"'; ?>><?php esc_html_e('Popular Courses', 'deep'); ?></option>
				<option value="<?php echo esc_attr( '3' ); ?>" <?php if ( $type == '3' ) echo 'selected="selected"'; ?>><?php esc_html_e('Top Rated Courses', 'deep'); ?></option>
				<option value="<?php echo esc_attr( '4' ); ?>" <?php if ( $type == '4' ) echo 'selected="selected"'; ?>><?php esc_html_e('Random Courses', 'deep'); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('count') ?>"><?php esc_html_e('Courses Count: ','deep') ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'current' ); ?>"><?php esc_html_e( 'Show current course', 'deep' ); ?></label>
		</p>
		<?php
	}
