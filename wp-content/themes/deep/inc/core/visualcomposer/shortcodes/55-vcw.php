<?php
if( ! class_exists( 'VCW_Constants' ) ) { 
    return;
}
$list_currency = array(
	'Bitcoin' => 'BTC',
	'US Dollar' => 'USD',
	'Eurozone Euro' => 'EUR',
	'Pound Sterling' =>  'GBP',
    'Japanese Yen' => 'JPY',
    'Canadian Dollar' => 'CAD',
    'Australian Dollar' => 'AUD',
    'Chinese Yuan' => 'CNY',
    'Swiss Franc' =>  'CHF',
    'Swedish Krona' => 'SEK',
    'New Zealand Dollar' => 'NZD',
    'South Korean Won' => 'KRW',
    'Bitcoin Cash' => 'BCH',
    'UAE Dirham' => 'AED',
    'Afghan Afghani' => 'AFN',
    'Allion' => 'ALL',
    'Armenian Dram' => 'AMD',
    'Netherlands Antillean Guilder' => 'ANG',
    'Angolan Kwanza' => 'AOA',
    'Argentine Peso' => 'ARS',
    'Aruban Florin' => 'AWG',
    'Azerbaijani Manat' => 'AZN',
    'Bosnia-Herzegovina Convertible Mark' => 'BAM',
    'Barbadian Dollar' => 'BBD',
    'Bangladeshi Taka' => 'BDT',
    'Bulgarian Lev' => 'BGN',
    'Bahraini Dinar' => 'BHD',
    'Burundian Franc' => 'BIF',
    'Bermudan Dollar' => 'BMD',
    'Brunei Dollar' => 'BND',
    'Bolivian Boliviano' => 'BOB',
    'Brazilian Real' => 'BRL',
    'BitSend' => 'BSD',
    'Bhutanese Ngultrum' => 'BTN',
    'Botswanan Pula' => 'BWP',
    'Belize Dollar' => 'BZD',
    'Congolese Franc' => 'CDF',
    'Chilean Unit of Account (UF)' => 'CLF',
    'Chilean Peso' => 'CLP',
    'Colombian Peso' => 'COP',
    'CrowdCoin' => 'CRC',
    'Cuban Peso' => 'CUP',
    'Cape Verdean Escudo' => 'CVE',
    'Czech Koruna' => 'CZK',
    'Djiboutian Franc' => 'DJF',
    'Danish Krone' => 'DKK',
    'Dominican Peso' => 'DOP',
    'Algerian Dinar' => 'DZD',
    'Egyptian Pound' => 'EGP',
    'Ethiopian Birr' => 'ETB',
    'Fijian Dollar' => 'FJD',
    'Falkland Islands Pound' => 'FKP',
    'Georgian Lari' => 'GEL',
    'Ghanaian Cedi' => 'GHS',
    'Gibraltar Pound' => 'GIP',
    'Gambian Dalasi' => 'GMD',
    'Guinean Franc' => 'GNF',
    'Guatemalan Quetzal' => 'GTQ',
    'Guyanaese Dollar' => 'GYD',
    'Hong Kong Dollar' => 'HKD',
    'Honduran Lempira' => 'HNL',
    'Croatian Kuna' => 'HRK',
    'Haitian Gourde' => 'HTG',
    'Hungarian Forint' => 'HUF',
    'Indonesian Rupiah' => 'IDR',
    'Israeli Shekel' => 'ILS',
    'Indian Rupee' => 'INR',
    'Iraqi Dinar' => 'IQD',
    'Iranian Rial' => 'IRR',
    'Icelandic Króna' => 'ISK',
    'Jersey Pound' => 'JEP',
    'Jamaican Dollar' => 'JMD',
    'Jordanian Dinar' => 'JOD',
    'Kenyan Shilling' => 'KES',
    'Kyrgystani Som' => 'KGS',
    'Cambodian Riel' => 'KHR',
    'Comorian Franc' => 'KMF',
    'North Korean Won' => 'KPW',
    'Kuwaiti Dinar' => 'KWD',
    'Cayman Islands Dollar' => 'KYD',
    'Kazakhstani Tenge' => 'KZT',
    'Laotian Kip' => 'LAK',
    'Lebanese Pound' => 'LBP',
    'Sri Lankan Rupee' => 'LKR',
    'Liberian Dollar' => 'LRD',
    'Lesotho Loti' => 'LSL',
    'Libyan Dinar' => 'LYD',
    'SatoshiMadness' => 'MAD',
    'Moldovan Leu' => 'MDL',
    'Malagasy Ariary' => 'MGA',
    'Macedonian Denar' => 'MKD',
    'Myanma Kyat' => 'MMK',
    'Mongolian Tugrik' => 'MNT',
    'Macanese Pataca' => 'MOP',
    'Mauritanian Ouguiya' => 'MRO',
    'Mauritian Rupee' => 'MUR',
    'Maldivian Rufiyaa' => 'MVR',
    'Malawian Kwacha' => 'MWK',
    'Mexican Peso' => 'MXN',
    'Malaysian Ringgit' => 'MYR',
    'Mozambican Metical' => 'MZN',
    'Namibian Dollar' => 'NAD',
    'Nigerian Naira' => 'NGN',
    'Autonio' => 'NIO',
    'Norwegian Krone' => 'NOK',
    'Nepalese Rupee' => 'NPR',
    'Omani Rial' => 'OMR',
    'Panamanian Balboa' => 'PAB',
    'Peruvian Nuevo Sol' => 'PEN',
    'Papua New Guinean Kina' => 'PGK',
    'Philippine Peso' => 'PHP',
    'Pakistani Rupee' => 'PKR',
    'Polish Zloty' => 'PLN',
    'Paraguayan Guarani' => 'PYG',
    'Qatari Rial' => 'QAR',
    'Romanian Leu' => 'RON',
    'Serbian Dinar' => 'RSD',
    'Russian Ruble' => 'RUB',
    'Rwandan Franc' => 'RWF',
    'Saudi Riyal' => 'SAR',
    'Steem Dollars' => 'SBD',
    'Seychellois Rupee' => 'SCR',
    'Sudanese Pound' => 'SDG',
    'Singapore Dollar' => 'SGD',
    'Saint Helena Pound' => 'SHP',
    'Sierra Leonean Leone' => 'SLL',
    'Somali Shilling' => 'SOS',
    'Surinamese Dollar' => 'SRD',
    'São Tomé and Príncipe Dobra' => 'STD',
    'Salvadoran Colón' => 'SVC',
    'Syrian Pound' => 'SYP',
    'Swazi Lilangeni' => 'SZL',
    'Thai Baht' => 'THB',
    'Tajikistani Somoni' => 'TJS',
    'Turkmenistani Manat' => 'TMT',
    'Tunisian Dinar' => 'TND',
    'TopCoin' => 'TOP',
    'Turkish Lira' => 'TRY',
    'Trinidad and Tobago Dollar' => 'TTD',
    'New Taiwan Dollar' => 'TWD',
    'Tanzanian Shilling' => 'TZS',
    'Ukrainian Hryvnia' => 'UAH',
    'Ugandan Shilling' => 'UGX',
    'Uruguayan Peso' => 'UYU',
    'Uzbekistan Som' => 'UZS',
    'Venezuelan Bolívar Fuerte' => 'VEF',
    'Vietnamese Dong' => 'VND',
    'Vanuatu Vatu' => 'VUV',
    'Samoan Tala' => 'WST',
    'CFA Franc BEAC' => 'XAF',
    'East Caribbean Dollar' => 'XCD',
    'CFA Franc BCEAO' => 'XOF',
    'CFP Franc' => 'XPF',
    'Yemeni Rial' => 'YER',
    'South African Rand' => 'ZAR',
    'Zambian Kwacha' => 'ZMW',
    'Zimbabwean Dollar' => 'ZWL',
    'Silver (troy ounce)' => 'XAG',
    'Xaucoin' => 'XAU',
    'Ethereum' => 'ETH',
    'Ripple' => 'XRP',
    'Litecoin' => 'LTC',
    'NEO' => 'NEO',
    'Cardano' => 'ADA',
    'Stellar' => 'XLM',
    'EOS' => 'EOS',
    'Monero' => 'XMR',
    'IOTA' => 'MIOTA',
    'Dash' => 'DASH',
    'NEM' => 'XEM',
    'TRON' => 'TRX',
    'Ethereum Classic' => 'ETC',
    'Tether' => 'USDT',
    'VeChain' => 'VEN',
    'Nano' => 'NANO',
    'Lisk' => 'LSK',
    'Qtum' => 'QTUM',
    'Bitgem' => 'BTG',
    'OmiseGO' => 'OMG',
    'ICON' => 'ICX',
    'Zcash' => 'ZEC',
    'DigixDAO' => 'DGD',
    'Binance Coin' => 'BNB',
    'Steem' => 'STEEM',
    'Verge' => 'XVG',
    'Stratis' => 'STRAT',
    'Bytecoin' => 'BCN',
    'Populous' => 'PPT',
    'Waves' => 'WAVES',
    'Siacoin' => 'SC',
    'RChain' => 'RHOC',
    'Maker' => 'MKR',
    'Dogecoin' => 'DOGE',
    'BitShares' => 'BTS',
    'Status' => 'SNT',
    'Decred' => 'DCR',
    'Aeternity' => 'AE',
    'Augur' => 'REP',
    'Waltonchain' => 'WTC',
    '0x' => 'ZRX',
    'Electroneum' => 'ETN',
    'Komodo' => 'KMD',
    'Veritaseum' => 'VERI',
    'Ark' => 'ARK',
    'Hshare' => 'HSR',
    'Ardor' => 'ARDR',
    'Bitmark' => 'BTM',
    'BatCoin' => 'BAT',
    'Ethos' => 'ETHOS',
    'KuCoin Shares' => 'KCS',
    'Dragonchain' => 'DRGN',
    'Syscoin' => 'SYS',
    'Gas' => 'GAS',
    'Cryptonex' => 'CNX',
    'Golem' => 'GNT',
    'PIVX' => 'PIVX',
    'DigiByte' => 'DGB',
    'Zilliqa' => 'ZIL',
    'MonaCoin' => 'MONA',
    'aelf' => 'ELF',
    'Revain' => 'R',
    'Aion' => 'AION',
    'Factom' => 'FCT',
    'Loopring' => 'LRC',
    'FunFair' => 'FUN',
    'Byteball Bytes' => 'GBYTE',
    'Nebulas' => 'NAS',
    'QASH' => 'QASH',
    'Particl' => 'PART',
    'ReddCoin' => 'RDD',
    'KingN Coin' => 'KNC',
    'GXShares' => 'GXS',
    'IOStoken' => 'IOST',
    'ZCoin' => 'XZC',
    'SALT' => 'SALT',
    'ChainLink' => 'LINK',
    'Power Ledger' => 'POWR',
    'Dent' => 'DENT',
    'Neblio' => 'NEBL',
    'Dentacoin' => 'DCN',
    'iCoin' => 'ICN',
    'Kin' => 'KIN',
    'Polymath' => 'POLY',
    'Bancor' => 'BNT',
    'Nxt' => 'NXT',
    'Enigma' => 'ENG',
    'Bitcore' => 'BTX',
    'Cindicator' => 'CND',
    'TenX' => 'PAY',
    'SmartCash' => 'SMART',
    'Blocknet' => 'BLOCK',
    'Request Network' => 'REQ',
    'Pillar' => 'PLR',
    'MaidSafeCoin' => 'MAID',
    'Vertcoin' => 'VTC',
    'SingularityNET' => 'AGI',
    'Gnosis' => 'GNO',
    'Emercoin' => 'EMC',
    'Nexus' => 'NXS',
    'WAX' => 'WAX',
    'Storj' => 'STORJ',
    'BitcoinDark' => 'BTCD',
    'GameCredits' => 'GAME',
    'Quantstamp' => 'QSP',
    'Genesis Vision' => 'GVT',
    'Enjin Coin' => 'ENJ',
    'SuperNET' => 'UNITY',
    'Raiden Network Token' => 'RDN',
    'Santiment Network Token' => 'SAN',
    'Ignis' => 'IGNIS',
    'ZenCash' => 'ZEN',
    'Theta Token' => 'THETA',
    'Civic' => 'CVC',
    'Monaco' => 'MCO',
    'Po.et' => 'POE',
    'Substratum' => 'SUB',
    'DigitalNote' => 'XDN',
    'NAV Coin' => 'NAV',
    'Skycoin' => 'SKY',
    'Decentraland' => 'MANA',
    'Storm' => 'STORM',
    'Aragon' => 'ANT',
    'Time New Bank' => 'TNB',
    'MediShares' => 'MDS',
    'iExec RLC' => 'RLC',
    'High Performance Blockchain' => 'HPB',
    'XPA' => 'XPA',
    'Achain' => 'ACT',
    'Ubiq' => 'UBQ',
    'BLOCKv' => 'VEE',
    'Pura' => 'PURA',
    'Dynamic Trading Rights' => 'DTR',
    'PayPie' =>'PPP', 
    'SophiaTX' =>'SPHTX', 
    'Metal' => 'MTL',
    'MediBloc' => 'MED',
    'ETHLend' => 'LEND',
    'BridgeCoin' => 'BCO',
    'Red Pulse' => 'RPX',
    'Experience Points' => 'XP',
    'AdEx' => 'ADX',
    'Oyster' => 'PRL',
    'Asch' => 'XAS',
    'MinexCoin' => 'MNX',
    'IoT Chain' => 'ITC',
    'BlazeCoin' => 'BLZ',
    'POA Network' => 'POA',
    'Telcoin' => 'TEL',
    'Nuls' => 'NULS',
    'DEW' => 'DEW',
    'VIBE' => 'VIBE',
    'Ambrosus' => 'AMB',
    'Simple Token' => 'OST',
    'BitBay' => 'BAY',
    'Edgeless' => 'EDG',
    'CRYPTO20' => 'C20', 
    'Quantum Resistant Ledger' => 'QRL',
    'WePower' => 'WPR',
    'Peercoin' => 'PPC',
    'Ink' => 'INK',
    'DATA' => 'DTA',
    'SpankChain' => 'SPANK',
    'DeepBrain Chain' => 'DBC',
    'Burst' => 'BURST',
    'Bibox Token' => 'BIX',
    'Melon' => 'MLN',
    'Eidoo' => 'EDO',
    'SONM' => 'SNM',
    'ION' => 'ION',
    'Wings' => 'WINGS',
    'Jibrel Network' => 'JNT',
    'Wagerr' => 'WGR',
    'AirSwap' => 'AST',
    'LBRY Credits' => 'LBC',
    'SaluS' => 'SLS',
    'Rcoin' => 'RCN',
    'AppCoins' => 'APPC',
    'Agoras Tokens' => 'AGRS',
    'SingularDTV' => 'SNGLS',
    'CloakCoin' => 'CLOAK',
    'WaBi' => 'WABI',
    'Streamr DATAcoin' => 'DATA',
    'Einsteinium' => 'EMC2',
    'XTRABYTES' => 'XBY',
    'MobileGo' => 'MGO',
    'Gifto' => 'GTO',
    'Cobinhood' => 'COB',
    'Metaverse ETP' => 'ETP',
    'Comet' => 'CMT',
    'INS Ecosystem' => 'INS',
    'Bread' => 'BRD',
    'SIRIN LABS Token' => 'SRN',
    'Feathercoin' => 'FTC',
    'HTMLCOIN' => 'HTML',
    'Viacoin' => 'VIA',
    'Gulden' => 'NLG',
    'Counterparty' => 'XCP',
    'Mobius' => 'MOBI',
    'district0x' => 'DNT',
    'adToken' => 'ADT',
    'Etherparty' => 'FUEL',
    'Delphy' => 'DPY',
    'UTRUST' => 'UTK',
    'TaaS' => 'TAAS',
    'OriginTrail' => 'TRAC',
    'Capricoin' => 'CPC',
    'Decision Token' => 'HST',
    'Modum' => 'MOD',
    'NAGA' => 'NGC',
    'Rise' => 'RISE',
    'Unikoin Gold' => 'UKG',
    'U.CASH' => 'UCASH',
    'BlockMason Credit Protocol' => 'BCPT',
    'Aeon' => 'AEON',
    'Bottos' => 'BTO',
    'Trinity Network Credit' => 'TNC',
    'CoinDash' => 'CDT',
    'Crown' => 'CRW',
    'Groestlcoin' => 'GRS',
    'QLINK' => 'QLC',
    'Lunyr' => 'LUN',
    'Centra' => 'CTR',
    'Tierion' => 'TNT',
    'DECENT' => 'DCT',
    'Medicalchain' => 'MTN',
    'Triggers' => 'TRIG',
    'Viberate' => 'VIB',
    'STK' => 'STK',
    'Shift' => 'SHIFT',
    'ECC' => 'ECC',
    'SunContract' => 'SNC',
    'SIBCoin' => 'SIB',
    'Safe Exchange Coin' => 'SAFEX',
    'ATMChain' => 'ATM',
    'Spectre.ai Dividend Token' => 'SXDT',
    'Humaniq' => 'HMQ',
    'Namecoin' => 'NMC',
    'Pepe Cash' => 'PEPECASH',
    'I/O Coin' => 'IOC',
    'Flash' => 'FLASH',
    'NetCoin' => 'NET',
    'COSS' => 'COSS',
    'TokenCard' => 'TKN',
    'Maecenas' => 'ART',
    'Cappasity' => 'CAPP',
    'Hive Project' => 'HVN',
    'Datum' => 'DAT',
    'PotCoin' => 'POT',
    'Everex' => 'EVX',
    'Cofound.it' => 'CFI',							    
    'Presearch' => 'PRE',
    'bitCNY' => 'BITCNY',
    'indaHash' => 'IDH',
    'Diamond' => 'DMD',
    'Lykke' => 'LKK',
    'ColossusCoinXT' => 'COLX',
    'ZClassic' => 'ZCL',
    'Blockport' => 'BPT',
    'Odyssey' => 'OCN',
    'DeepOnion' => 'ONION',
    'VeriCoin' => 'VRC',
    'Dimecoin' => 'DIME',
    'Bean Cash' => 'BITB',
    'Mercury' => 'MER',
    'WhiteCoin' => 'XWC',
    'All Sports' => 'SOC',
    'Unobtanium' => 'UNO',
    'Soarcoin' => 'SOAR',
    'FairGame' => 'FAIR',
    'BlackCoin' => 'BLK',							    
    'Voxels' => 'VOX',
    'WeTrust' => 'TRST',
    'Swarm' => 'SWM',
    'Synereo' => 'AMP',
    'DADI' => 'DADI',
    'Lamden' => 'TAU',
    'NoLimitCoin' => 'NLC2',
    'Internet Node Token' => 'INT',
    'Mintcoin' => 'MINT',
    'Monetha' => 'MTH',
    'YOYOW' => 'YOYOW',
    'Matchpool' => 'GUP',
    'Zeusshield' => 'ZSC',
    'Numeraire' => 'NMR',
    'Elastic' => 'XEL',
    'Aeron' => 'ARN',
    'FirstBlood' => '1ST',
    'Energo' => 'TSL',
    'Blocktix' => 'TIX',
    'SwftCoin' => 'SWFTC',
    'Agrello' => 'DLT',
    'Mothership' => 'MSP',
    'Bloom' => 'BLT',
    'LEOcoin' => 'LEO',
    'Zeepin' => 'ZPT',
    'BitConnect' => 'BCC',
    'Expanse' => 'EXP',
    'Aurora DAO' => 'AURA',
    'HempCoin' => 'THC',
    'SwissBorg' => 'CHSB',
    'Grid+' => 'GRID',
    'Mooncoin' => 'MOON',
    'Global Tour Coin' => 'GTC',
    'Pascal Coin' => 'PASC',
    'Cubits' => 'QBT',
    'Trade Token' => 'TIO',
    'GridCoin' => 'GRC',
    'QunQun' => 'QUN',
    'Omni' => 'OMNI',
    'EncrypGen' => 'DNA',
    'Selfkey' => 'KEY',
    'Uquid Coin' => 'UQC',
    'Zoin' => 'ZOI',
    'Electra' => 'ECA',
    'ALQO' => 'ALQO',
    'Moeda Loyalty Points' => 'MDA',
    'Olympus Labs' => 'MOT',
    'Covesting' => 'COV',
    'Ormeus Coin' => 'ORME',
    'OKCash' => 'OK',
    'carVertical' => 'CV',
    'Content and AD Network' => 'CAN',
    'Tao' => 'XTO',
    'OAX' => 'OAX',
    'KickCoin' => 'KICK',
    'Catcoin' => 'CAT',
    'LATOKEN' => 'LA',
    'Radium' => 'RADS',
    'Databits' => 'DTB',
    'Waves Community Token' => 'WCT',
    'Blackmoon' => 'BMC',
    'PoSW Coin' => 'POSW',
    'MonetaryUnit' => 'MUE',
    'SolarCoin' => 'SLR',
    'Propy' => 'PRO',
    'InsurePal' => 'IPL',
    'Phore' => 'PHR',
    'Dai' => 'DAI',
    'AirToken' => 'AIR',
    'Primecoin' => 'XPM',
    'E-Dinar Coin' => 'EDR',
    'Hackspace Capital' => 'HAC',
    'Everus' => 'EVR',
    'DomRaider' => 'DRT',
    'Divi' => 'DIVX',
    'FedoraCoin' => 'TIPS',
    'Neumark' => 'NEU',
    'Rubycoin' => 'RBY',
    'SHIELD' => 'XSH',
    'ALIS' => 'ALIS',
    'Swarm City' => 'SWT',
    'NewYorkCoin' => 'NYC',
    'BitDegree' => 'BDG',
    'Chronobank' => 'TIME',
    'Musicoin' => 'MUSIC',
    'Universal Currency' => 'UNIT',
    'iXledger' => 'IXT',
    'Rivetz' => 'RVT',
    'Nexium' => 'NXC',
    'Myriad' => 'XMY',
    'Energycoin' => 'ENRG',
    'Golos' => 'GOLOS',
    'Auroracoin' => 'AUR',
    'LoMoCoin' => 'LMC',
    'Spectrecoin' => 'XSPEC',
    'DecentBet' => 'DBET',
    'Hubii Network' => 'HBT',
    'ICOS' => 'ICOS',
    'ToaCoin' => 'TOA',
    'NeosCoin' => 'NEOS',
    'Target Coin' => 'TGT',
    'Hacken' => 'HKN',
    'Bodhi' => 'BOT',
    'Peerplays' => 'PPY',
    'Open Trading Network' => 'OTN',
    'ATBCoin' => 'ATB',
    'PutinCoin' => 'PUT',
    'Clams' => 'CLAM',
    'FlorinCoin' => 'FLO',
    'Primas' => 'PST',
    'InvestFeed' => 'IFT',
    'Paragon' => 'PRG',
    'BitDice' => 'CSNO',
    'Neutron' => 'NTRN',
    'Boolberry' => 'BBR',
    'Etheroll' => 'DICE',
    'Stox' => 'STX',
    'Syndicate' => 'SYNX',
    'GET Protocol' => 'GET',
    'Polybius' => 'PLBT',
    'Gambit' => 'GAM',
    'Ties.DB' => 'TIE',
    'ProChain' => 'PRA',
    'Snovio' => 'SNOV',
    'Quantum' => 'QAU',
    'Global Currency Reserve' => 'GCR',
    'Incent' => 'INCNT',
    'Bismuth' => 'BIS',
    'bitUSD' => 'BITUSD',
    'OneRoot Network' => 'RNT',
    'Hedge' => 'HDG',
    'DubaiCoin' => 'DBIX',
    'Worldcore' => 'WRC',
    'Patientory' => 'PTOY',
    'Exchange Union' => 'XUC',
    'Pareto Network' => 'PARETO',
    'Dynamic' => 'DYN',
    'Solaris' => 'XLR',
    'PRIZM' => 'PZM',
    'LUXCoin' => 'LUX',
    'OracleChain' => 'OCT',
    'EarthCoin' => 'EAC',
    'Espers' => 'ESP',
    'bitqy' => 'BQ',
    'Pandacoin' => 'PND',
    'E-coin' => 'ECN',
    'Circuits of Value' => 'COVAL',
    'Xaurum' => 'XAUR',
    'FoldingCoin' => 'FLDC',
    'Hydro Protocol' => 'HOT',
    'TransferCoin' => 'TX',
    'aXpire' => 'AXP',
    'NVO' => 'NVST',
    'CHIPS' => 'CHIPS',							    
    'Pluton' => 'PLU',
    'Bounty0x' => 'BNTY',
    'CoinFi' => 'COFI',
    'Mysterium' => 'MYST',
    'CargoX' => 'CXO',
    'BLUE' => 'BLUE',
    'PinkCoin' => 'PINK',
    'B2BX' => 'B2B',
    'LockChain' => 'LOC',
    'DCORP' => 'DRP',
    'Leverj' => 'LEV',
    'Kore' => 'KORE',
    'The ChampCoin' => 'TCC',
    'CoinMeet' => 'MEE',
    'Remicoin' => 'RMC',
    'CVCoin' => 'CVCOIN',
    'MyBit Token' => 'MYB',
    'Internet of People' => 'IOP',
    'Ethorse' => 'HORSE',
    'Oxycoin' => 'OXY',
    'Curecoin' => 'CURE',
    'Measurable Data Token' => 'MDT',
    'Payfair' => 'PFR',
    'Aventus' => 'AVT',
    'Playkey' => 'PKT',
    'Polis'	  => 'POLIS', 
    'SportyCo' => 'SPF',
    'Rebellious' => 'REBL',
    'Linda' => 'LINDA',
    'Bitcrystals' => 'BCY',
    'BitcoinZ' => 'BTCZ',
    'Stealthcoin' => 'XST',
    'GoByte' => 'GBX',
    'OBITS' => 'OBITS',
    'NuBits' => 'USNBT',
    'Sphere' => 'SPHR',
    'Credo' => 'CREDO',
    'Spectre.ai Utility Token' => 'SXUT',
    'ClearPoll' => 'POLL',
    'Novacoin' => 'NVC',
    'LOCIcoin' => 'LOCI',
    'Flixxo' => 'FLIXX',
    'Elixir' => 'ELIX',
    'Pirl' => 'PIRL',
    'Devery' => 'EVE',
    'Sequence' => 'SEQ',
    'Karma' => 'KRM',
    'ArtByte' => 'ABY',
    'Vcash' => 'XVC',
    'Voise' => 'VOISE',
    'Zilla' => 'ZLA',
    'Atmos' => 'ATMS',
    'AidCoin' => 'AID',
    'AICHAIN' => 'AIT',
    'Breakout Stake' => 'BRX',
    'HEAT' => 'HEAT',
    'HyperStake' => 'HYP',
    'Eroscoin' => 'ERO',
    'SureRemit' => 'RMT',
    'Block Array' => 'ARY',
    'DopeCoin' => 'DOPE',
    'Change' => 'CAG',
    'Gatcoin' => 'GAT',
    'eBitcoin' => 'EBTC',
    'HelloGold' => 'HGT',
    'Qwark' => 'QWARK',
    'vTorrent' => 'VTR',
    'Synergy' => 'SNRG',
    'EuropeCoin' => 'ERC',
    'Zeitcoin' => 'ZEIT',
    'Memetic / PepeCoin' => 'MEME',
    'HEROcoin' => 'PLAY',
    'EchoLink' => 'EKO',
    'Sumokoin' => 'SUMO',
    'RussiaCoin' => 'RC',
    'BunnyCoin' => 'BUN',
    'VeriumReserve' => 'VRM',
    'Denarius' => 'DNR',
    'Lampix' => 'PIX',
    'Ethereum Movie Venture' => 'EMV',
    'Elite' => '1337',
    'TrueFlip' => 'TFL',
    'Pesetacoin' => 'PTC',
    'Bitcoin Plus' => 'XBC',
    'Travelflex' => 'TRF',
    'GoldCoin' => 'GLD',
    'Sharechain' => 'SSS',
    'Iungo' => 'ING',
    'Aigang' => 'AIX',
    'GCN Coin' => 'GCN',
    'ZrCoin' => 'ZRC',
    'Hush' => 'HUSH',
    'Mercury Protocol' => 'GMT',
    'APX' => 'APX',
    'adbank' => 'ADB',
    'Viuly' => 'VIU',
    'Breakout' => 'BRK',
    'Dotcoin' => 'DOT',
    'Monkey Project' => 'MONK',
    'Uniform Fiscal Object' => 'UFO',
    'Publica' => 'PBL',
    'Sociall' => 'SCL',
    'Bulwark' => 'BWK',
    '2GIVE' => '2GIVE',
    'Creditbit' => 'CRB',
    'AdShares' => 'ADST',
    'Farad' => 'FRD',
    'Monoeci' => 'XMCC',
    'BetaCoin' => 'BET',
    'Blockpool' => 'BPL',
    'ExclusiveCoin' => 'EXCL',
    'Dovu' => 'DOVU',
    'Sprouts' => 'SPRTS',
    'LuckChain' => 'BASH',
    'Verify' => 'CRED',
    'Opus' => 'OPT',
    'FLiK' => 'FLIK',
    'Bela' => 'BELA',
    'MCAP' => 'MCAP',
    'MyWish' => 'WISH',
    'Blitzcash' => 'BLITZ',
    'Yocoin' => 'YOC',
    'DigiPulse' => 'DGPT',
    'Global Jobcoin' => 'GJC',
    'Internxt' => 'INXT',
    'Tokes' => 'TKS',
    'Creativecoin' => 'CREA',
    'Primalbase Token' => 'PBT',
    'MAZA' => 'MZC',
    'AudioCoin' => 'ADC',
    'DNotes' => 'NOTE',
    'Bonpay' => 'BON',
    'Bitcloud' => 'BTDX',
    'Riecoin' => 'RIC',
    'Altcoin' => 'ALT',
    'CannabisCoin' => 'CANN',
    'Project Decorum' => 'PDC',
    'Wild Crypto' => 'WILD',
    'Masternodecoin' => 'MTNC',
    'TeslaCoin' => 'TES',
    'XGOX' => 'XGOX',
    'HunterCoin' => 'HUC',
    'Hawala.Today' => 'HAT',
    'GoldMint' => 'MNTP',
    'EventChain' => 'EVC',
    'Vsync' => 'VSX',
    'Obsidian' => 'ODN',
    'REX' => 'REX',
    'Karbo' => 'KRB',
    'TrustPlus' => 'TRUST',
    'TrezarCoin' => 'TZC',
    'Zero' => 'ZER',
    'EverGreenCoin' => 'EGC',
    'SpreadCoin' => 'SPR',
    'Tracto' => 'TRCT',
    'NuShares' => 'NSR',
    'LIFE' => 'LIFE',
    'Social Send' => 'SEND',
    'Bitswift' => 'SWIFT',
    'ATLANT' => 'ATL',
    'HollyWoodCoin' => 'HWC',
    'Privatix' => 'PRIX',
    'Bitzeny' => 'ZNY',
    'vSlice' => 'VSL',
    'Bowhead' => 'AHT',
    'Upfiring' => 'UFR',
    'Goodomy' => 'GOOD',
    'FlypMe' => 'FYP',
    'Sexcoin' => 'SXC',
    'Coinlancer' => 'CL',
    'ChainCoin' => 'CHC',
    'e-Gulden' => 'EFL',
    'Terracoin' => 'TRC',
    'BuzzCoin' => 'BUZZ',
    'Startcoin' => 'START',
    'Qvolta' => 'QVT',
    'Magi' => 'XMG',
    'Ergo' => 'EFYT',
    'Regalcoin' => 'REC',
    'BlueCoin' => 'BLU',
    'Giga Watt Token' => 'WTT',
    'Zephyr' => 'ZEPH',
    'Quark' => 'QRK',
    'Ellaism' => 'ELLA',
    'Indorse Token' => 'IND',
    'Embers' => 'MBRS',
    'GanjaCoin' => 'MRJA',
    'Innova' => 'INN',
    'DraftCoin' => 'DFT',
    'MicroMoney' => 'AMM',
    'Speed Mining Service' => 'SMS',
    'Ignition' => 'IC',
    'ParkByte' => 'PKB',
    'Maggie' => 'MAG',
    'Darcrus' => 'DAR',
    'IntenseCoin' => 'ITNS',
    'Condensate' => 'RAIN',
    'Rupee' => 'RUP',
    'Jesus Coin' => 'JC',
    'bitJob' => 'STU',
    'LiteDoge' => 'LDOGE',
    'Smartlands' => 'SLT',
    'eBoost' => 'EBST',
    'Interstellar Holdings' => 'HOLD',
    'CampusCoin' => 'CMPCO',
    'Oceanlab' => 'OCL',
    'PiplCoin' => 'PIPL',
    'Crypto Bullion' => 'CBX',
    'SmileyCoin' => 'SMLY',
    'IncaKoin' => 'NKA',
    'STRAKS' => 'STAK',
    'ProCurrency' => 'PROC',
    'FORCE' => 'FOR',
    'Ethbits' => 'ETBS',
    'onG.social' => 'ONG',
    'LendConnect' => 'LCT',
    'Greencoin' => 'GRE',
    'GuccioneCoin' => 'GCC',
    'Chronologic' => 'DAY',
    'Adzcoin' => 'ADZ',
    'Bitdeal' => 'BDL',
    'Hexx' => 'HXX',
    'MagicCoin' => 'MAGE',
    'EquiTrader' => 'EQT',
    'Pylon Network' => 'PYLNT',
    'ACChain' => 'ACC',
    'ELTCOIN' => 'ELTCOIN',
    'POLY AI' => 'AI',
    'CryptoPing' => 'PING',
    'UnbreakableCoin' => 'UNB',
    'FundYourselfNow' => 'FYN',
    'Linx' => 'LINX',
    'OP Coin' => 'OPC',
    'DigitalPrice' => 'DP',
    'Orbitcoin' => 'ORB',
    'Unify' => 'UNIFY',
    'Polcoin' => 'PLC',
    '42-coin' => '42',
    'Footy Cash' => 'XFT',
    'Bytecent' => 'BYC',
    "Miners' Reward Token" => 'MRT',
    'SkinCoin' => 'SKIN',
    'InflationCoin' => 'IFLT',
    'Cream' => 'CRM',
    'Renos' => 'RNS',
    'NobleCoin' => 'NOBL',
    'KekCoin' => 'KEK',
    'Photon' => 'PHO',
    'Growers International' => 'GRWI',
    'Unity Ingot' => 'UNY',
    'ArbitrageCT' => 'ARCT',
    'Canada eCoin' => 'CDN',
    'Pure' => 'PURE',
    'Fastcoin' => 'FST',
    'Universe' => 'UNI',
    'Dinastycoin' => 'DCY',
    'Moin' => 'MOIN',
    'Shorty' => 'SHORTY',
    'Legends Room' => 'LGD',
    'ArcticCoin' => 'ARC',
    'Zennies' => 'ZENI',
    'FujiCoin' => 'FJC',
    'Woodcoin' => 'LOG',
    'Unitus' => 'UIS',
    'InsaneCoin' => 'INSN',
    'Etheriya' => 'RIYA',
    'PetroDollar' => 'XPD',
    'Bata' => 'BTA',
    'ICO OpenLedger' => 'ICOO',
    'Zetacoin' => 'ZET',
    'Emphy' => 'EPY',
    'KiloCoin' => 'KLC',
    'PopularCoin' => 'POP',
    'Abjcoin' => 'ABJ',
    'VIVO' => 'VIVO',
    'HOdlcoin' => 'HODL',
    'Bitradio' => 'BRO',
    'LeviarCoin' => 'XLC',
    'ERA' => 'ERA',
    'BitBar' => 'BTB',
    'Machinecoin' => 'MAC',
    'Intelligent Trading Tech' => 'ITT',
    'Skeincoin' => 'SKC',
    'Kolion' => 'KLN',
    'Piggycoin' => 'PIGGY',
    'BiblePay' => 'BBP',
    'Jetcoin' => 'JET',
    'Copico' => 'XCPO',
    'Guncoin' => 'GUN',
    'RouletteToken' => 'RLT',
    'Centurion' => 'CNT',
    'Ethereum Cash' => 'ECASH',
    'Kubera Coin' => 'KBR',
    'Suretly' => 'SUR',
    'CryptoCarbon' => 'CCRB',
    'Bitcoin Scrypt' => 'BTCS',
    'Digitalcoin' => 'DGC',
    'Rupaya [OLD]' => 'RUPX',
    'Soma' => 'SCT',
    'Monster Byte' => 'MBI',
    'WandX' => 'WAND',
    'DigiCube' => 'CUBE',
    'QubitCoin' => 'Q2C',
    'Elcoin' => 'EL',
    'Scorecoin' => 'SCORE',
    'Megacoin' => 'MEC',
    'Ace' => 'ACE',
    'BritCoin' => 'BRIT',
    'FuckToken' => 'FUCK',
    'Grimcoin' => 'GRIM',
    'Authorship' => 'ATS',
    'SmartCoin' => 'SMC',
    'Deutsche eMark' => 'DEM',
    'WhaleCoin' => 'WHL',
    'Sugar Exchange' => 'SGR',
    'Cryptojacks' => 'CJ',
    'TittieCoin' => 'TTC',
    'Equal' => 'EQL',
    'Elementrem' => 'ELE',
    'Happycoin' => 'HPC',
    'LanaCoin' => 'LANA',
    'FirstCoin' => 'FRST',
    'Sovereign Hero' => 'HERO',
    'DFSCoin' => 'DFS',
    'Influxcoin' => 'INFX',
    'Triangles' => 'TRI',
    'GlassCoin' => 'GLS',
    'ERC20' => 'ERC20',
    'Bit20' => 'BTWTY',
    'Kurrent' => 'KURT',
    'Minereum' => 'MNE',
    'WavesGo' => 'WGO',
    'BitTokens' => 'BXT',
    'Nyancoin' => 'NYAN',
    'Billionaire Token' => 'XBL',
    'Netko' => 'NETKO',
    'MojoCoin' => 'MOJO',
    'Royal Kingdom Coin' => 'RKC',
    'TrumpCoin' => 'TRUMP',
    'Rimbit' => 'RBT',
    'CarTaxi Token' => 'CTX',
    'Bitcoin Red' => 'BTCRED',
    'Ammo Reloaded' => 'AMMO',
    'AmsterdamCoin' => 'AMS',
    'Titcoin' => 'TIT',
    'SecureCoin' => 'SRC',
    'Truckcoin' => 'TRK',
    'DROXNE' => 'DRXNE',
    'Pakcoin' => 'PAK',
    'Cryptonite' => 'XCN',
    'Ethereum Gold' => 'ETG',
    'Steneum Coin' => 'STN',
    'ENTCash' => 'ENT',
    'Gapcoin' => 'GAP',
    'Phoenixcoin' => 'PXC',
    'Octanox' => 'OTX',
    'iEthereum' => 'IETH',
    'Pioneer Coin' => 'PCOIN',
    'CryptoInsight' => 'TKR',
    'Motocoin' => 'MOTO',
    'Joulecoin' => 'XJO',
    'SwagBucks' => 'BUCKS',
    'HomeBlockCoin' => 'HBC',
    'bitBTC' => 'BITBTC',
    'DaxxCoin' => 'DAXX',
    'Marscoin' => 'MARS',
    'eBitcoinCash' => 'EBCH',
    'HiCoin' => 'XHI',
    'Desire' => 'DSR',
    'Visio' => 'VISIO',
    'Garlicoin' => 'GRLC',
    'Litecoin Plus' => 'LCP',
    'TagCoin' => 'TAG',
    'CoinonatX' => 'XCXT',
    'TOKYO' => 'TOKC',
    'Cypher' => 'CYP',
    'Prototanium' => 'PR',
    '808Coin' => '808',
    'Newbium' => 'NEWB',
    'Fujinto' => 'NTO',
    'bitSilver' => 'BITSILVER',
    'Advanced Internet Blocks' => 'AIB',
    'ZoZoCoin' => 'ZZC',
    'Confido' => 'CFD',
    'Crystal Clear ' => 'CCT',
    'ChanCoin' => 'CHAN',
    'Huncoin' => 'HNC',
    'PostCoin' => 'POST',
    'Bolivarcoin' => 'BOLI',
    'Roofs' => 'ROOFS',
    'MACRON' => 'MCRN',
    'CannaCoin' => 'CCN',
    'Atomic Coin' => 'ATOM',
    'Ethereum Dark' => 'ETHD',
    'Mao Zedong' => 'MAO',
    'EcoCoin' => 'ECO',
    'BipCoin' => 'BIP',
    'Shadow Token' => 'SHDW',
    'Pascal Lite' => 'PASL',
    'Kayicoin' => 'KAYI',
    'SagaCoin' => 'SAGA',
    'LeaCoin' => 'LEA',
    'Onix' => 'ONX',
    'C-Bit' => 'XCT',
    'RedCoin' => 'RED',
    'BERNcash' => 'BERN',
    'LiteBar' => 'LTB',
    'Honey' => 'HONEY',
    'Dollarcoin' => 'DLC',
    'SpaceCoin' => 'SPACE',
    'BitCoal' => 'COAL',
    'EOT Token' => 'EOT',
    'Quebecoin' => 'QBC',
    'SoonCoin' => 'SOON',
    'GlobalBoost-Y' => 'BSTY',
    'GlobalToken' => 'GLT',
    'Bitcoin Fast' => 'BCF',
    'Dalecoin' => 'DALC',
    'Senderon' => 'SDRN',
    'BROTHER' => 'BRAT',
    'VoteCoin' => 'VOT',
    'Veltor' => 'VLT',
    'Qbic' => 'QBIC',
    'SixEleven' => '611',
    'Lightning Bitcoin [Futures]' => 'LBTC',
    'PoSToken' => 'POS',
    'EagleCoin' => 'EAGLE',
    'AllSafe' => 'ASAFE2',
    'Cashcoin' => 'CASH',
    'CacheCoin' => 'CACH',
    'ReeCoin' => 'REE',
    'Speedcash' => 'SCS',
    'Luna Coin' => 'LUNA',
    'YENTEN' => 'YTN',
    'FinCoin' => 'FNC',
    'ICOBID' => 'ICOB',
    'Gold Pressed Latinum' => 'GPL',
    'Cannation' => 'CNNC',
    'Dix Asset' => 'DIX',
    'Madcoin' => 'MDC',
    'Spots' => 'SPT',
    'GPU Coin' => 'GPU',
    'X-Coin' => 'XCO',
    'Zlancer' => 'ZCG',
    'HempCoin' => 'HMP',
    'GoldPieces' => 'GP',
    'JavaScript Token' => 'JS',
    'Eryllium' => 'ERY',
    'Kronecoin' => 'KRONE',
    'BenjiRolls' => 'BENJI',
    'FuzzBalls' => 'FUZZ',
    'Neuro' => 'NRO',
    'Master Swiscoin' => 'MSCN',
    'LiteCoin Ultra' => 'LTCU',
    'WomenCoin' => 'WOMEN',
    'SONO' => 'ALTCOM',
    'VapersCoin' => 'VPRC',
    'Save and Gain' => 'SANDG',
    'Wild Beast Block' => 'WBB',
    'QuazarCoin' => 'QCN',
    'HealthyWormCoin' => 'WORM',
    'Iconic' => 'ICON',
    'AnarchistsPrime' => 'ACP',
    'GeertCoin' => 'GEERT',
    'Rawcoin' => 'XRC',
    'RSGPcoin' => 'RSGP',
    'Bitvolt' => 'VOLT',
    'GBCGoldCoin' => 'GBC',
    'Veros' => 'VRS',
    'Coupecoin' => 'COUPE',
    'CrevaCoin' => 'CREVA',
    'PRCoin' => 'PRC',
    'Aerium' => 'AERM',
    'Vault Coin' => 'VLTC',
    'Project-X' => 'NANOX',
    'EXRNchain' => 'EXRN',
    'CaliphCoin' => 'CALC',							    
    'Hi Mutual Society' => 'HMC',
    'DIBCOIN' => 'DIBC',
    'Digital Money Bits' => 'DMB',
    'PACcoin' => 'PAC',
    'Rialto' => 'XRL',
    'Ecobit' => 'ECOB',
    'Xenon' => 'XNN',
    'Prospectors Gold' => 'PGL',
    'Starbase' => 'STAR',
    'AsiaCoin' => 'AC',
    'Cryptopay' => 'CPAY',
    'Virtacoin' => 'VTA',
    'BitBoost' => 'BBT',
    'REAL' => 'REAL',
    'Starta' => 'STA',
    'EncryptoTel [ETH]' => 'ETT',
    'Ixcoin' => 'IXC',
    'YashCoin' => 'YASH',
    'MarteXcoin' => 'MXT',
    'Anoncoin' => 'ANC',
    'Carboncoin' => 'CARBON',
    'FIMKrypto' => 'FIMK',
    'Version' => 'V',
    'LeafCoin' => 'LEAF',
    'AurumCoin' => 'AU',
    'StarCredits' => 'STRC',
    'Bitpark Coin' => 'BPC',
    'ShadowCash' => 'SDC',
    'FlutterCoin' => 'FLT',
    'WorldCoin' => 'WDC',
    'InPay' => 'INPAY',
    'I0Coin' => 'I0C',
    'MetalCoin' => 'METAL',
    'Commodity Ad Network' => 'CDX',
    'MaxCoin' => 'MAX',
    'NEVERDIE' => 'NDC',
    'Fantomcoin' => 'FCN',
    'PlatinumBAR' => 'XPTX',
    'HitCoin' => 'HTC',
    'EthBet' => 'EBET',
    'Aricoin' => 'ARI',
    'Trollcoin' => 'TROLL',
    'iTicoin' => 'ITI',
    'Kobocoin' => 'KOBO',
    'Rustbits' => 'RUSTBITS',
    'Link Platform' => 'LNK',
    'HoboNickels' => 'HBN',
    'Datacoin' => 'DTC',
    'UniCoin' => 'UNIC',
    'Ultimate Secure Cash' => 'USC',
    'Bitstar' => 'BITS',
    'CryptoForecast' => 'CFT',
    'Bitcurrency' => 'BTCR',
    'FuelCoin' => 'FC2',
    'Opal' => 'OPAL',
    'Halcyon' => 'HAL',
    'BTCtalkcoin' => 'TALK',
    'UltraCoin' => 'UTC',
    'GAIA' => 'GAIA',
    'PayCoin' => 'XPY',
    'Argentum' => 'ARG',
    'WayGuide' => 'WAY',
    'GoldReserve' => 'XGR',
    'Valorbit' => 'VAL',
    'PureVidz' => 'VIDZ',
    'Signatum' => 'SIGT',
    'Flycoin' => 'FLY',
    'Manna' => 'MANNA',
    'Mineum' => 'MNM',
    'GoldBlocks' => 'GB',
    'Tigercoin' => 'TGC',
    'Blakecoin' => 'BLC',
    'DigitalDevelopersFund' => 'DDF',
    'SACoin' => 'SAC',
    '8Bit' => '8BIT',
    'Sterlingcoin' => 'SLG',
    'Tattoocoin (Standard Edition)' => 'TSE',
    'BlockPay' => 'BLOCKPAY',
    'Colossuscoin V2' => 'CV2',
    'SuperCoin' => 'SUPER',
    'Coin2.1' => 'C2',
    'AquariusCoin' => 'ARCO',
    'ChessCoin' => 'CHESS',
    'Coin(O)' => 'CNO',
    'Virta Unique Coin' => 'VUC',
    'KushCoin' => 'KUSH',
    'GlobalCoin' => 'GLC',
    'Chronos' => 'CRX',
    'FUNCoin' => 'FUNC',
    'Bitz' => 'BITZ',
    'BigUp' => 'BIGUP',
    'Emerald Crypto' => 'EMD',
    'Swing' => 'SWING',
    'Ccore' => 'CCO',							    
    'OctoCoin' => '888',
    'PX' => 'PX',
    'Dashcoin' => 'DSH',
    'Evil Coin' => 'EVIL',
    'Virtacoinplus' => 'XVP',
    'Rubies' => 'RBIES',
    'Ratecoin' => 'XRA',
    'VirtualCoin' => 'VC',
    'Prime-XI' => 'PXI',
    'AmberCoin' => 'AMBER',
    'Joincoin' => 'J',
    'Freicoin' => 'FRC',
    'RevolverCoin' => 'XRE',
    'Sativacoin' => 'STV',
    'Philosopher Stones' => 'PHS',
    'bitGold' => 'BITGOLD',
    'NevaCoin' => 'NEVA',
    'TEKcoin' => 'TEK',
    'Bankcoin' => 'B@',
    'Independent Money System' => 'IMS',
    'SproutsExtreme' => 'SPEX',
    'Xios' => 'XIOS',
    'BumbaCoin' => 'BUMBA',
    'GameUnits' => 'UNITS',
    'Jin Coin' => 'JIN',
    'Eurocoin' => 'EUC',
    'SecretCoin' => 'SCRT',
    'IslaCoin' => 'ISL',
    'ParallelCoin' => 'DUO',
    'Nekonium' => 'NUKO',
    'Quatloo' => 'QTL',
    'BoostCoin' => 'BOST',
    'Yacoin' => 'YAC',
    'RonPaulCoin' => 'RPC',
    'StarCash Network' => 'STARS',
    'MustangCoin' => 'MST',
    'BriaCoin' => 'BRIA',
    'Beatcoin' => 'XBTS',
    'Darsek' => 'KED',
    'Enigma' => 'XNG',
    'Bitcoin 21' => 'XBTC21',
    'Evotion' => 'EVO',
    'PayCon' => 'CON',
    'Acoin' => 'ACOIN',
    'Zurcoin' => 'ZUR',
    'Bitcoin Planet' => 'BTPL',
    'Asiadigicoin' => 'ADCN',
    'Mincoin' => 'MNC',
    'TajCoin' => 'TAJ',
    'Money' => '$$$',
    'Creatio' => 'XCRE',
    'Flaxscript' => 'FLAX',
    'Franko' => 'FRK',
    'Cthulhu Offerings' => 'OFF',
    '300 Token' => '300',
    'Impact' => 'IMX',
    'SOILcoin' => 'SOIL',
    'Crypto' => 'CTO',
    'Firecoin' => 'FIRE',
    'Debitcoin' => 'DBTC',
    'CompuCoin' => 'CPN',
    'AntiBitcoin' => 'ANTI',
    'Bolenum' => 'BLN',
    'Bitcedi' => 'BXC',
    'Elacoin' => 'ELC',
    'bitEUR' => 'BITEUR',
    'BitQuark' => 'BTQ',
    'BnrtxCoin' => 'BNX',
    'Ripto Bux' => 'RBX',
    'BitAsean' => 'BAS',
    'Coinonat' => 'CXT',
    'Marijuanacoin' => 'MAR',
    'Digital Rupees' => 'DRS',
    'Trident Group' => 'TRDT',
    'Californium' => 'CF',
    'VIP Tokens' => 'VIP',
    'SongCoin' => 'SONG',
    'Litecred' => 'LTCR',
    'VectorAI' => 'VEC2',
    'WARP' => 'WARP',
    'Slevin' => 'SLEVIN',
    'Network Token' => 'NTWK',
    'Metal Music Coin' => 'MTLMC3',
    'Uro' => 'URO',
    'Printerium' => 'PRX',
    'Artex Coin' => 'ATX',
    'BillaryCoin' => 'BLRY',
    'DAPPSTER' => 'DLISK',
    'MiloCoin' => 'MILO',
    'Theresa May Coin' => 'MAY',
    'Shilling' => 'SH',
    'Jewels' => 'JWL',
    'iDice' => 'ICE',
    'Sojourn' => 'SOJ',
    'MindCoin' => 'MND',
    'Ride My Car' => 'RIDE',
    'PonziCoin' => 'PONZI',
    'Cabbage' => 'CAB',
    'Unrealcoin' => 'URC',
    'Zayedcoin' => 'ZYD',
    'JobsCoin' => 'JOBS',
    'ARbit' => 'ARB',
    'Blackstar' => 'BSTAR',
    'Braincoin' => 'BRAIN',
    'TAGRcoin' => 'TAGR',
    'ZetaMicron' => 'ZMC',
    'EGO' => 'EGO',
    'Useless Ethereum Token' => 'UET',
    'Dreamcoin' => 'DRM',
    'PosEx' => 'PEX',
    'CryptoEscudo' => 'CESC',
    'Pulse' => 'PULSE',
    'CryptoWorldX Token' => 'CWXT',
    'Solarflarecoin' => 'SFC',
    'PIECoin' => 'PIE',
    'Orlycoin' => 'ORLY',
    'ExchangeN' => 'EXN',
    'G3N' => 'G3N',
    'Xonecoin' => 'XOC',
    'Steps' => 'STEPS',
    'BowsCoin' => 'BSC',
    'ImpulseCoin' => 'IMPS',
    'OsmiumCoin' => 'OS76',
    'High Voltage' => 'HVCO',
    'PlayerCoin' => 'PLACO',
    'Destiny' => 'DES',
    'BOAT' => 'BOAT',
    'LetItRide' => 'LIR',
    'BiosCrypto' => 'BIOS',
    'PLNcoin' => 'PLNC',
    'CRTCoin' => 'CRT',
    'CoExistCoin' => 'COXST',
    'Credence Coin' => 'CRDNC',
    'Zonecoin' => 'ZNE',
    'iBank' => 'IBANK',
    'Tychocoin' => 'TYCHO',
    'AgrolifeCoin' => 'AGLC',
    'Dollar Online' => 'DOLLAR',
    'BioBar' => 'BIOB',
    'Torcoin' => 'TOR',
    'SydPak' => 'SDP',
    'Argus' => 'ARGUS',
    'Elysium' => 'ELS',
    'Coimatic 3.0' => 'CTIC3',
    'P7Coin' => 'P7C',
    'SocialCoin' => 'SOCC',
    'Ulatech' => 'ULA',
    'Antilitecoin' => 'ALTC',
    'NodeCoin' => 'NODC',
    'FuturXe' => 'FXE',
    'Concoin' => 'CONX',
    'Selfiecoin' => 'SLFI',
    'Magnum' => 'MGM',
    'Coimatic 2.0' => 'CTIC2',
    'GeyserCoin' => 'GSR',
    'LevoPlus' => 'LVPS',
    'PizzaCoin' => 'PIZZA',
    'Tristar Coin' => 'TSTR',
    'CCMiner' => 'CCM100',
    'Digital Credits' => 'DGCS',
    'Abncoin' => 'ABN',
    'Ebittree Coin' => 'EBT',
    'Huobi Token' => 'HT',
    'ATMCoin' => 'ATMC',
    'Arcblock' => 'ABT',
    'True Chain' => 'TRUE',
    'Elastos' => 'ELA',
    'EDUCare' => 'EKT',
    'Nucleus Vision' => 'NCASH',
    'Cube' => 'AUTO',
    'Ruff' => 'RUFF',
    'Bitcoin Diamond' => 'BCD',
    'Experty' => 'EXY',
    'OFCOIN' => 'OF',
    'LinkEye' => 'LET',
    'Electronic PK Chain' => 'EPC',
    'AI Doctor' => 'AIDOC',
    'OceanChain' => 'OC',
    'Animation Vision Cash' => 'AVH',
    'TopChain' => 'TOPC',
    'Credits' => 'CS',
    'Acute Angle Cloud' => 'AAC',
    'Matryx' => 'MTX',
    'Genaro Network' => 'GNX',
    'Wi Coin' => 'WIC',
    'IHT Real Estate Protocol' => 'IHT',
    'ChatCoin' => 'CHAT',
    'LightChain' => 'LIGHT',
    'W3Coin' => 'W3C',
    'SmartMesh' => 'SMT',
    'Pundi X' => 'PXS',
    'Refereum' => 'RFR',
    'CFun' => 'CFUN',
    'Rock' => 'RKT',
    'Litecoin Cash' => 'LCC',
    'HalalChain' => 'HLC',
    'Nework' => 'NKC',
    'Show' => 'SHOW',
    'StarChain' => 'STC',
    'Hyper Pay' => 'HPY',
    'United Bitcoin' => 'UBTC',
    'Yee' => 'YEE',
    'MktCoin' => 'MLM',
    'Bankex' => 'BKX',
    'RealChain' => 'RCT',
    'THEKEY' => 'TKY',
    'Bee Token' => 'BEE',
    'MergeCoin' => 'MGC',
    'UG Token' => 'UGT',
    'Fargocoin' => 'FRGC',
    'BOScoin' => 'BOS',
    'TokenClub' => 'TCT',
    'Gems ' => 'GEM',
    'Matrix AI Network' => 'MAN',
    'AWARE' => 'AWR',
    'COMSA [XEM]' => 'CMS',
    'BitcoinX' => 'BCX',
    'Kcash' => 'KCASH',
    'BitSoar' => 'BSR',
    'InfChain' => 'INF',
    'Scry.info' => 'DDD',
    'SpaceChain' => 'SPC',
    'ShareX' => 'SEXC',
    'ValueChain' => 'VLC',
    'Super Bitcoin' => 'SBTC',
    'Republic Protocol' => 'REN',
    'Read' => 'READ',
    'PressOne' => 'PRS',
    'Fusion' => 'FSN',
    'Molecular Future' => 'MOF',
    'iQuant' => 'IQT',
    'Tezos (Pre-Launch)' => 'XTZ',
    'Insights Network' => 'INSTAR',
    'RefToken' => 'REF',
    'Envion' => 'EVN',
    'ATN' => 'ATN',
    'Remme' => 'REM',
    'Infinity Economics' => 'XIN',
    'SelfSell' => 'SSC',
    'Filecoin [Futures]' => 'FIL',
    'Swisscoin' => 'SIC',
    'Restart Energy MWAT' => 'MWAT',
    'Universa' => 'UTNP',
    'Lympo' => 'LYM',
    'ugChain' => 'UGC',
    'DMarket' => 'DMT',
    'Tidex Token' => 'TDX',
    'MOAC' => 'MOAC',
    'Titanium Blockchain' => 'BAR',
    'UnlimitedIP' => 'UIP',
    'Aidos Kuneen' => 'ADK',
    'BigONE Token' => 'BIG',
    'ClubCoin' => 'CLUB',
    'Ink Protocol' => 'XNK',
    'Qube' => 'QUBE',
    'International Diamond' => 'XID',
    'Fortuna' => 'FOTA',
    'SegWit2x' => 'B2X',
    'ZenGold' => 'ZENGOLD',
    'IPChain' => 'IPC',
    'MSD' => 'MSD',
    'Maverick Chain' => 'MVC',
    'Candy' => 'CANDY',
    'Sether' => 'SETH',
    'GeoCoin' => 'GEO',
    'Bitcoin White' => 'BTW',
    'Neurotoken' => 'NTK',
    'Etherecash' => 'ECH',
    'Crypterium' => 'CRPT',
    'Alphacat' => 'ACAT',
    'Dether' => 'DTH',
    'InvestDigital' => 'IDT',
    'WETH' => 'WETH',
    'StrikeBitClub' => 'SBC',
    'FidentiaX' => 'FDX',
    'Numus' => 'NMS',
    'TimesCoin' => 'TMC',
    'StrongHands' => 'SHND',
    'WINCOIN' => 'WC',
    'Tokenbox' => 'TBX',
    'VPNCoin' => 'VASH',
    'EDRCoin' => 'EDRC',
    'BlockCDN' => 'BCDN',
    'Cashaa' => 'CAS',
    'Copytrack' => 'CPY',
    'CasinoCoin' => 'CSC',
    'Zap' => 'ZAP',
    'Infinitecoin' => 'IFC',
    'Tigereum' => 'TIG',
    'Tokugawa' => 'TOK',
    'Datawallet' => 'DXT',
    'Bitair' => 'BTCA',
    'Bezop' => 'BEZ',
    'Shekel' => 'JEW',
    'Jiyo' => 'JIYO',
    'Jingtum Tech' => 'SWTC',
    'B3Coin' => 'KB3',
    'EA Coin' => 'EAG',
    'Golfcoin' => 'GOLF',
    'MUSE' => 'MUSE',
    'CryptopiaFeeShares' => 'CEFS',
    'GOLD Reward Token' => 'GRX',
    'DRP Utility' => 'DRPU',
    'Superior Coin' => 'SUP',
    'SwapToken' => 'TOKEN',
    'DIMCOIN' => 'DIM',
    'TechShares' => 'THS',
    'Golos Gold' => 'GBG',
    'WA Space' => 'WA',
    'Crave' => 'CRAVE',
    'Kzcash' => 'KZC',
    'BT2 [CST]' => 'BT2',
    'AvatarCoin' => 'AV',
    'Escroco' => 'ESC',
    'Cyder' => 'CYDER',
    'Gladius Token' => 'GLA',
    'Monero Gold' => 'XMRG',
    'Internet of Things' => 'XOT',
    'DavorCoin' => 'DAV',
    'Sand Coin' => 'SND',
    'Cashme' => 'CME',
    'BioCoin' => 'BIO',
    'GameChain System' => 'GCS',
    'Royalties' => 'XRY',
    'DynamicCoin' => 'DMC',
    'President Trump' => 'PRES',
    'Alphabit' => 'ABC',
    'United Traders Token' => 'UTT',
    'Animecoin' => 'ANI',
    'SIGMAcoin' => 'SIGMA',
    'Macro' => 'MCR',
    'GAY Money' => 'GAY',
    'TeslaCoilCoin' => 'TESLA',
    'Sparks' => 'SPK',
    'PeepCoin' => 'PCN',
    'Phantomx' => 'PNX',
    'Bitcoin God' => 'GOD',
    'TerraNova' => 'TER',
    'EtherSportz' => 'ESZ',
    'Galactrum' => 'ORE',
    'SHACoin' => 'SHA',
    'Vezt' => 'VZT',
    'BitSerial' => 'BTE',
    'President Johnson' => 'GARY',
    'LandCoin' => 'LDCN',
    'Harvest Masternode Coin' => 'HC',
    'PinkDog' => 'PDG',
    'Cloud' => 'CLD',
    'IrishCoin' => 'IRL',
    'Triaconta' => 'TRIA',
    'GrandCoin' => 'GDC',
    'First Bitcoin Capital' => 'BITCF',
    'Ethereum Lite' => 'ELITE',
    'IDEX Membership' => 'IDXM',
    'AlpaCoin' => 'APC',
    'STEX' => 'STEX',
    'LePen' => 'LEPEN',
    'OX Fina' => 'OX',
    'Vulcano' => 'VULC',
    'Storjcoin X' => 'SJCX',
    'ZSEcoin' => 'ZSE',
    'Bitcoin Atom' => 'BCA',
    'WeAreSatoshi' => 'WSX',
    'GoldMaxCoin' => 'GMX',
    'FlappyCoin' => 'FLAP',
    'Indicoin' => 'INDI',
    'Safe Trade Coin' => 'XSTC',
    'BTCMoon' => 'BTCM',
    'TurboCoin' => 'TURBO',
    'NumusCash' => 'NUMUS',
    'Sakuracoin' => 'SKR',
    'UniversalRoyalCoin' => 'UNRC',
    'DeusCoin' => 'DEUS',
    'Protean' => 'PRN',
    'Avoncoin' => 'ACN',
    'Bastonet' => 'BSN',
    'Francs' => 'FRN',
    'Tellurion' => 'TELL',
    'NEO GOLD' => 'NEOG',
    'HTML5COIN' => 'HTML5',
    'Quotient' => 'XQN',
    'First Bitcoin' => 'BIT',
    'Nitro' => 'NOX',
    'Primulon' => 'PRIMU',
    'NamoCoin' => 'NAMO',
    'Dutch Coin' => 'DUTCH',
    'netBit' => 'NBIT',
    'BlazerCoin' => 'BLAZR',
    'Zilbercoin' => 'ZBC',
    'KlondikeCoin' => 'KDC',
    'SISA' => 'SISA',
    'LinkedCoin' => 'LKC',
    'SJWCoin' => 'SJW',
    'PlexCoin' => 'PLX',
    'Fonziecoin' => 'FONZ',
    'CoffeeCoin' => 'CFC',
    'RabbitCoin' => 'RBBT',
    'Slothcoin' => 'SLOTH',
    'Halloween Coin' => 'HALLO',
    'Wowcoin' => 'WOW',
    'RichCoin' => 'RICHX',
    'RoyalCoin' => 'ROYAL',
    'Akuya Coin' => 'AKY',
    'Sense' => 'SENSE',
    'SnakeEyes' => 'SNAKE',
    'CORION' => 'COR',
    'Sharkcoin' => 'SAK',
    'MarxCoin' => 'MARX',
    'India Coin' => 'INDIA',
    'Aces' => 'ACES',
    'Donationcoin' => 'DON',
    'MoneyCoin' => 'MONEY',
    'Minex' => 'MINEX',
    'Levocoin' => 'LEVO',
    'Cheapcoin' => 'CHEAP',
    'Wink' => 'WINK',
    'Runners' => 'RUNNERS',
    'TeamUp' => 'TEAM',
    'Omicron' => 'OMC',
    'MobileCash' => 'MBL',
    'HODL Bucks' => 'HDLB',
    'PokeCoin' => 'POKE',
    'BestChain' => 'BEST',
    'UR' => 'UR',
    'Magnetcoin' => 'MAGN',
    'SafeCoin' => 'SFE',
    'Digital Bullion Gold' => 'DBG',
    'Bitbase' => 'BTBc',
    'Natcoin' => 'NTC',
    'Compcoin' => 'CMP',
    'Qora' => 'QORA',
    'Topaz Coin' => 'TOPAZ',
    'Dashs' => 'DASHS',
    'eGold' => 'EGOLD',
    'Fazzcoin' => 'FAZZ',
    'FireFlyCoin' => 'FFC',
    'Opescoin' => 'OPES',
    'RHFCoin' => 'RHFC',
    'Pirate Blocks' => 'SKULL',
    'Lazaruscoin' => 'LAZ',
    'DarkLisk' => 'DISK',
    'Antimatter' => 'ANTX',
    'Peerguess' => 'GUESS',
    'UNCoin' => 'UNC',
    'FAPcoin' => 'FAP',
    'ANRYZE' => 'RYZ',
    'Birds' => 'BIRDS',
    'SportsCoin' => 'SPORT',
    'XTD Coin' => 'XTD',
    'ShellCoin' => 'SHELL',
    'T-coin' => 'TCOIN',
    'UGAIN' => 'GAIN',
    'EggCoin' => 'EGG',
    'Granite' => 'GRN',
    'Karmacoin' => 'KARMA',
    'Voyacoin' => 'VOYA',
    'PayPeer' => 'PAYP',
    'KashhCoin' => 'KASHH',
    'RubleBit' => 'RUBIT',
    'TheCreed' => 'TCR',
    'TodayCoin' => 'TODAY',
    'MMXVI' => 'MMXVI',
    'High Gain' => 'HIGH',
    'Happy Creator Coin' => 'HCC',
    'Musiconomi' => 'MCI',
    'X2' => 'X2',
    'Bitok' => 'BITOK',
    'CyberCoin' => 'CC',
    'GameLeagueCoin' => 'GML',
    'Cycling Coin' => 'CYC',
    'Hyper' => 'HYPER',
    'Axiom' => 'AXIOM',
    'BitAlphaCoin' => 'BAC',
    'The Vegan Initiative' => 'XVE',
    'Smoke' => 'SMOKE',
    'DeltaCredits' => 'DCRE',
    'Dubstep' => 'DUB',
    'Yescoin' => 'YES',
    'FutCoin' => 'FUTC',
    'FrankyWillCoin' => 'FRWC',
    'Tattoocoin (Limited Edition)' => 'TLE',
    'Aseancoin' => 'ASN',
    'Regacoin' => 'REGA',
    'Bubble' => 'BUB',
    'Moneta' => 'MONETA',
    'Psilocybin' => 'PSY',
    'PrismChain' => 'PRM',
    'TrickyCoin' => 'TRICK',
    'InvisibleCoin' => 'IVZ',
    'Operand' => 'OP',
    'LAthaan' => 'LTH',
    'Pabyosi Coin (Special)' => 'PCS',
    'BITFID' => 'FID',
    'LLToken' => 'LLT',
    'OCOW' => 'OCOW',
    'Farstcoin' => 'FRCT',
);

vc_map( array(
    'name'          =>'Virtual Coins',
    'base'          => 'vcw',
    "description"   => "Shortcodes for Virtual Coin Widgets Plugin",
    'category'      => esc_html__( 'Webnus Shortcodes', 'deep' ),
    "icon"          => "webnus-pricing-plan",
    'params'        => array(
        array(
            'type'          => 'dropdown',
            'heading'       => esc_html__( 'Type', 'deep' ),
            'param_name'    => 'type',
            'value'         => array(
                esc_html__( 'Convertor', 'deep' ) =>'1',
                esc_html__( 'Full Card', 'deep' ) =>'2',						
            ),
            'description' => esc_html__( 'Select the Virtual Coin Widgets type', 'deep')
        ),
        // Convertor
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Symbol #1', 'deep' ),
            'param_name'     => 'symbol_1',
            'description'    => esc_html__( 'Select the top currency', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'BTC', 
            'dependency'     => array(
                'element'       => 'type',
                'value'         => '1',)
        ),
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Symbol #2', 'deep' ),
            'param_name'     => 'symbol_2',
            'description'    => esc_html__( 'Select the bottom currency', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'USD', 
            'dependency'     => array(
                'element'       => 'type',
                'value'         => '1',)
        ),

        // Full Card
        array(
            'type'			=> 'dropdown',
            'heading'		=>	esc_html__( 'Columns', 'deep' ),
            'param_name'	=>	'columns',
            'description'   =>	esc_html__( 'Select the number of columns', 'deep'),
            'value'			=>	array('1','2','3','4','5'),
            'dependency'     => array(
                'element'       => 'type',
                'value'         => '2',)
        ),
        array(
            'type'           => 'attach_image',
            'heading'        => esc_html__( 'Symbol #1', 'deep' ),
            'param_name'     => 'img_f1',
            'value'          =>'',
            'description'    => esc_html__( 'Symbol Image', 'deep'),
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('1','2','3','4','5'),
            ),
        ),
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Symbol #1', 'deep' ),
            'param_name'     => 'symbol_f1',
            'description'    => esc_html__( 'Cryptocurrency for the widget', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'BTC', 
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('1','2','3','4','5'),
            ),
        ),
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Price Currency #1', 'deep' ),
            'param_name'     => 'currency_f1',
            'description'    => esc_html__( 'Currency used for price quote', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'USD', 
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('1','2','3','4','5'),
            ),
        ),						
        array(
            'type'           => 'attach_image',
            'heading'        => esc_html__( 'Symbol #2', 'deep' ),
            'param_name'     => 'img_f2',
            'value'          =>'',
            'description'    => esc_html__( 'Symbol Image', 'deep'),
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('2','3','4','5'),
            ),
        ),
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Symbol #2', 'deep' ),
            'param_name'     => 'symbol_f2',
            'description'    => esc_html__( 'Cryptocurrency for the widget', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'BTC', 
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('2','3','4','5'),
            ),
        ),
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Price Currency #2', 'deep' ),
            'param_name'     => 'currency_f2',
            'description'    => esc_html__( 'Currency used for price quote', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'USD', 
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('2','3','4','5'),
            ),
        ),							
        array(
            'type'           => 'attach_image',
            'heading'        => esc_html__( 'Symbol #3', 'deep' ),
            'param_name'     => 'img_f3',
            'value'          =>'',
            'description'    => esc_html__( 'Symbol Image', 'deep'),
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('3','4','5'),
            ),
        ),
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Symbol #3', 'deep' ),
            'param_name'     => 'symbol_f3',
            'description'    => esc_html__( 'Cryptocurrency for the widget', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'BTC', 
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('3','4','5'),
            ),
        ),
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Price Currency #3', 'deep' ),
            'param_name'     => 'currency_f3',
            'description'    => esc_html__( 'Currency used for price quote', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'USD', 
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('3','4','5'),
            ),
        ),						
        array(
            'type'           => 'attach_image',
            'heading'        => esc_html__( 'Symbol #4', 'deep' ),
            'param_name'     => 'img_f4',
            'value'          =>'',
            'description'    => esc_html__( 'Symbol Image', 'deep'),
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('4','5'),
            ),
        ),
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Symbol #4', 'deep' ),
            'param_name'     => 'symbol_f4',
            'description'    => esc_html__( 'Cryptocurrency for the widget', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'BTC', 
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('4','5'),
            ),
        ),
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Price Currency #4', 'deep' ),
            'param_name'     => 'currency_f4',
            'description'    => esc_html__( 'Currency used for price quote', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'USD', 
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('4','5'),
            ),
        ),							
        array(
            'type'           => 'attach_image',
            'heading'        => esc_html__( 'Symbol #5', 'deep' ),
            'param_name'     => 'img_f5',
            'value'          =>'',
            'description'    => esc_html__( 'Symbol Image', 'deep'),
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('5'),
            ),
        ),
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Symbol #5', 'deep' ),
            'param_name'     => 'symbol_f5',
            'description'    => esc_html__( 'Cryptocurrency for the widget', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'BTC', 
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('5'),
            ),
        ),
        array(
            'type'           => 'dropdown',
            'heading'        => esc_html__( 'Price Currency #5', 'deep' ),
            'param_name'     => 'currency_f5',
            'description'    => esc_html__( 'Currency used for price quote', 'deep'),
            'value'			 => $list_currency,
            'std'         	=> 'USD', 
            'dependency'     => array(
                'element'	=> 'columns', 
                'value'	=> array('5'),
            ),
        ),	
        // Class & ID 
        array(
            'group'			=> 'Class & ID',
            'type'			=> 'textfield',
            'heading'		=> esc_html__( 'Extra Class', 'deep' ),
            'param_name'	=> 'shortcodeclass',
            'value'			=> '',
            'edit_field_class'	=> 'vc_col-sm-6 vc_column paddingtop',
        ),
        array(
            'group'			=> 'Class & ID',
            'type'			=> 'textfield',
            'heading'		=> esc_html__( 'ID', 'deep' ),
            'param_name'	=> 'shortcodeid',
            'value'			=> '',
            'edit_field_class'	=> 'vc_col-sm-6 vc_column paddingtop',
        ),
    ),
) );