		...
		spotedImage.onload = function() {
			...
			self.element.delegate('span', 'click', function(event) {
				...
				if (widget.config.interactivity === 'none') {
					var htmlBuilt = $('<div id="ihwt-hotspot-dot-'+counter+'" data-index="'+counter+'"><i class="delete-item fa fa-close"></i></div>');
				} else {
					var htmlBuilt = $('<div id="ihwt-hotspot-dot-'+counter+'" data-index="'+counter+'"><i class="delete-item fa fa-close"></i></div>').addClass(widget.config.hiddenClass);
				}
	...
	Hotspot.prototype.popupWindow = function(index, currentData) {
		...
		for (var i = 0; i < dataStuff.length; i++) {
			var val = dataStuff[i],
				...
				input = '<label>'+val.property+'</label><textarea class="ihwt-hotspot-'+val.property+'">'+defaultText+'</textarea>';
			
			if(val.property == 'Title') {
				input = '<label>'+val.property+'</label><input type="text" class="ihwt-hotspot-'+val.property+'" value="'+defaultText+'" />';
			}
			
			$popupInnerHtml += '<div>'+input+'</div>';
		};
		
		$popupInnerHtml += '<a href="#" title="'+closeText+'" class="wn-hotspot-close vc_general vc_ui-button vc_ui-button-default vc_ui-button-shape-rounded vc_ui-button-fw ihwt-hotspot-close">'+closeText+'</a><a href="#" title="'+saveText+'" class="wn-hotspot-save vc_general vc_ui-button vc_ui-button-action vc_ui-button-shape-rounded vc_ui-button-fw ihwt-hotspot-save">'+saveText+'</a>';
		
		var $popupHtml = '<div class="ihwt-hotspot-popup"><div class="ihwt-hotspot-popup-title">'+popupTitle+'<a href="#" title="Close" class="ihwt-hotspot-close Default-close"></a></div>'+$popupInnerHtml+'</div>';
		
		$('body').append($popupHtml);
		
		$('.ihwt-hotspot-save').on('click', function() {
			var $container = $(this).parents('.ihwt-hotspot-popup'),
				dataBuild = {};
		...
		$('body').on('click', '.ihwt-hotspot-close', function() {
			$('.ihwt-hotspot-popup').remove();
		});
	};
	...
	Hotspot.prototype.beautifyData = function() {
		...
		for (var i = obj.length - 1; i >= 0; i--) {
			...
			if (this.config.interactivity === 'none') {
				var htmlBuilt = $('<div id="ihwt-hotspot-dot-'+el.index+'" data-index="'+el.index+'"><i class="delete-item fa fa-close"></i></div>');
			} else {
				var htmlBuilt = $('<div id="ihwt-hotspot-dot-'+el.index+'" data-index="'+el.index+'"><i class="delete-item fa fa-close"></i></div>').addClass(this.config.hiddenClass);
			}
