...
window.DeepCarouselTab = vc.shortcode_view.extend({
	...
	initialize:function (params) {
            window.DeepCarouselTab.__super__.initialize.call(this, params);
            _.bindAll(this, 'stopSorting');
        },
	render: function () {
		window.DeepCarouselTab.__super__.render.call(this);
		this.$tabs = this.$el.find('.wpb_tabs_holder');
        this.createAddTabButton();
		return this;
	},
	ready: function (e) {
		window.DeepCarouselTab.__super__.ready.call(this, e);
    ...
    addTab:function (e) {
            ...
            var tab_title = 'Item',
                tabs_count = this.$tabs.find('[data-element_type=content_carousel_tab]').length,
                tab_new_title = tab_title + ' ' + ( tabs_count + 1 ),
                tab_id = (+new Date() + '-' + tabs_count + '-' + Math.floor(Math.random() * 11));
            vc.shortcodes.create({shortcode:'content_carousel_tab', params:{title:tab_new_title, tab_id:tab_id}, parent_id:this.model.id});
            return false;
        },
       ...
       changedContent:function (view) {
            ...
            //console &&  console.log(' DeepCarouselTab :  changedContent save');
        },
   cloneModel:function (model, parent_id, save_order) {
            ...
            if (model.get('shortcode') === 'content_carousel_tab') _.extend(new_params, {tab_id:+new Date() + '-' + this.$tabs.find('[data-element-type=content_carousel_tab]').length + '-' + Math.floor(Math.random() * 11)});
            model_clone = Shortcodes.create({shortcode:model.get('shortcode'), id:vc_guid(), parent_id:parent_id, order:new_order, cloned:(model.get('shortcode') === 'content_carousel_tab' ? false : true), cloned_from:model.toJSON(), params:new_params});
...
jQuery(document).ready(function() {
    jQuery("body").on("change", ".disp_icon", function() {
        ...
        if(op=="Disables"){
            jQuery(this).parent().parent().parent().find('.content_carousel_tabicon_notice').css({display: "none"});
        }else{
            jQuery(this).parent().parent().parent().find('.content_carousel_tabicon_notice').css({display: "block"}); 
        }
    });
});