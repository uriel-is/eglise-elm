...
window.DeepContentSlider = vc.shortcode_view.extend({
	...
	initialize:function (params) {
            window.DeepContentSlider.__super__.initialize.call(this, params);
            _.bindAll(this, 'stopSorting');
        },
	render: function () {
		window.DeepContentSlider.__super__.render.call(this);
		this.$tabs = this.$el.find('.wpb_tabs_holder');
        this.createAddTabButton();
		return this;
	},
	ready: function (e) {
		window.DeepContentSlider.__super__.ready.call(this, e);
    ...
    addTab:function (e) {
            ...
            var tab_title = 'Slide',
                tabs_count = this.$tabs.find('[data-element_type=single_slide]').length,
                tab_new_title = tab_title + ' ' + ( tabs_count + 1 ),
                tab_id = (+new Date() + '-' + tabs_count + '-' + Math.floor(Math.random() * 11));
            vc.shortcodes.create({shortcode:'single_slide', params:{title:tab_new_title, tab_id:tab_id}, parent_id:this.model.id});
            return false;
        },
       ...
       changedContent:function (view) {
            ...
            //console &&  console.log(' DeepContentSlider :  changedContent save');
        },
   cloneModel:function (model, parent_id, save_order) {
            ...
            if (model.get('shortcode') === 'single_slide') _.extend(new_params, {tab_id:+new Date() + '-' + this.$tabs.find('[data-element-type=single_slide]').length + '-' + Math.floor(Math.random() * 11)});
            model_clone = Shortcodes.create({shortcode:model.get('shortcode'), id:vc_guid(), parent_id:parent_id, order:new_order, cloned:(model.get('shortcode') === 'single_slide' ? false : true), cloned_from:model.toJSON(), params:new_params});
