...
if ( $enable_animation_pro ) :
	...
	$script = '
		...
		*/
	// minifyed
	$script = ' <script> ( function( $ ) { $( document ).ready( function() { var controller = new ScrollMagic.Controller(); new ScrollMagic.Scene({ triggerElement: "#wpb_single_image' . $uniqid . '", duration: "' . $duration . '", triggerHook: "' . $trigger_hook . '" }) .setTween( "#wpb_single_image' . $uniqid . ' img", { ' . $opacity . $rotation . $horizontal_movement . $vertical_movement . ' } ) .addTo( controller ); }); })( jQuery ); </script>';
endif;
