...
if ( ! class_exists( 'WN_Frontend' ) ) {
    class WN_Frontend {
		...
		public function __construct() {
			...
			add_action( 'init', array( $this, 'update_child_theme_version' ) );
		}
        ...
        public function enqueue_styles() {
			...
			if ( $deep_minifier == '1' ) {
				...
				wp_enqueue_style( 'wn-iconfonts', DEEP_ASSETS_URL . 'css/frontend/base/07-iconfonts.css' );
				wp_enqueue_style( 'wn-blog', DEEP_ASSETS_URL . 'css/frontend/base/08-blog.css' );
		...
		* @since   4.0.1
		*/
		public function update_child_theme_version() {
			if ( is_child_theme() == true ) {
				$child_version = wp_get_theme()->Version;
				$parent_version = 'Version: ' . wp_get_theme()->parent()->Version;
			
				if ( $child_version < $parent_version  ) {
					$style = get_stylesheet_uri();
					$contents = wp_remote_fopen($style);
					
					if ( $style ) {				
						$searchfor = 'Version';
						$pattern = preg_quote($searchfor, '/');
						$pattern = "/^.*$pattern.*\$/m";
						if(preg_match_all($pattern, $contents, $version)){	
							$contents	= str_replace($version[0], $parent_version,$contents);
							$style_path = realpath( get_stylesheet_directory() . '/style.css' );
							file_put_contents($style_path, $contents);
						} 				
					} 
				} 
			}
