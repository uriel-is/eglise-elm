...
if ( ! class_exists( 'WN_Setup_Wizard' ) ) :
	class WN_Setup_Wizard {
		...
		public function __construct() {
			if ( ! is_admin() || empty( $_GET['page'] ) || $_GET['page'] !== 'webnus-setup-wizard' || ! current_user_can( 'manage_options' ) ) {
				return;
			}
		...
		public function check_purchase_code() {
			if ( get_option( 'deep_purchase_validation', '' ) != 'success' ) {
				echo '
				<div class="wsw-invalid-license-alert">
					<h2>' . esc_html__( 'Please enter valid purchase code', 'deep' ) . '</h2>
					<a href="' . admin_url( 'index.php?page=webnus-setup-wizard' ) . '">' . esc_html__( 'go to purchase code step', 'deep' ) . '</a>
				</div>';
				return true;
			}
			return false;
