...
<div class="archive-course-wrap clearfix">
	...
	<?php
	if ( is_search() ) {
		echo sprintf( esc_html__( 'Search Results: &ldquo;%s&rdquo;', 'deep' ), get_search_query() );
	if ( get_query_var( 'paged' ) ) {
		echo sprintf( esc_html__( '&nbsp;&ndash; Page %s', 'deep' ), get_query_var( 'paged' ) ); }
	} elseif ( is_tax() ) {
		echo single_term_title( '', false );
	}else{
		echo esc_html__('All Courses','deep');
	}
	?>
	</h2></div></section>
	<main class="container content llms-content w-course-archive">
		...
		<aside class="col-md-3">
			<div class="filter-category">
				<h3> <?php esc_html_e('COURSE CATEGORIES','deep'); ?> </h3>
			...
			<div class="row">
				<div class="col-sm-12 courses-sorting">
					...
					<form method="get" class="course-sorting-form w-nice-select" action="<?php echo esc_url( home_url( '/' ) ); ?>">
						...
						<div class="course-sorting-wrap clearfix">
							<span><?php esc_html_e('Sort by:', 'deep') ?></span>
							<select class="orderby-field w-nice-select" name="orderby">
								<option class="cs-orderby-option" value="w-order-course=dateNewest"><?php esc_html_e('Release Date (newest first)', 'deep') ?></option>
								<option class="cs-orderby-option" value="w-order-course=dateOldest"><?php esc_html_e('Release Date (oldest first)', 'deep') ?></option>
								<option class="cs-orderby-option" value="w-order-course=trending"><?php esc_html_e('Most Visiting', 'deep'); ?></option>
								<option class="cs-orderby-option" value="w-order-course=topRated"><?php esc_html_e('Top Rated', 'deep'); ?></option>
								<option class="cs-orderby-option" value="w-order-course=titleAZ"><?php esc_html_e('Course Title (a-z)', 'deep'); ?></option>
								<option class="cs-orderby-option" value="w-order-course=titleZA"><?php esc_html_e('Course Title (z-a)', 'deep'); ?></option>
							</select>
							<div class="switch-field">
								<!-- all -->
								<input type="radio" id="all_courses" value="" name="courses_price_filter" checked>
								<label for="all_courses"><?php esc_html_e('All', 'deep'); ?></label>
								<!-- paid -->
								<input type="radio" id="paid_courses" value="w-price-course=paidCourses" name="courses_price_filter">
								<label for="paid_courses"><?php esc_html_e('Paid', 'deep'); ?></label>
								<!-- free -->
								<input type="radio" id="free_courses" value="w-price-course=freeCourses" name="courses_price_filter">
								<label for="free_courses"><?php esc_html_e('Free', 'deep'); ?></label>
							</div>
				...
				<div class="col-md-12">
					<div class="filter-category-dropdown nice-select">
						<span class="current"> <?php esc_html_e('COURSE CATEGORIES','deep'); ?> </span>
						<?php the_widget ('Michigan_Course_Categories', 'post_counts=true&category_icon=true'); ?>
					</div>
				</div>
			</div>
				...
				<article class="w-course-list">
					...
					<div class="col-md-4 course-list-border-right">
						...
						<?php if ( has_post_thumbnail( $post->ID ) ) {
							$img = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'deep_blog2_img' );
						...
						<div class="course-list-content">
							...
							?>
								<div class="llms-price-wrapper">
									<div class="course-list-price"><?php deep_michigan_course_price(); ?></div>
								</div>
							<?php
							echo '<p>'.deep_excerpt(36).'</p>';
							?>
						</div>
					</div>
					</div>
						...
						<div class="col-md-4 course-list-border-right">
							<div class="course-list-review">
								<div class="modern-content">
									...
									?>
										<div class="llms-price-wrapper">
											<h4 class="llms-price"><span><?php deep_michigan_course_price(); ?></span></h4>
										</div>
									<?php
									?>
									<div class="clearfix modern-meta">
										...
										echo ($course_students)?'<span class="modern-students" title="'.esc_attr__('Enrolled Students','deep').'"><i class="sl-people"></i>'.$course_students.'</span>':'<span class="modern-viewers" title="'.esc_attr('Viewers','deep').'"><i class="fa fa-eye"></i>'.deep_getViews(get_the_ID()).'</span>';
										echo '</div>';
										?>
									</div>
								</div>
						...
						<div class="col-md-8 nopad-all none-grid">
							<div class="course-list-meta">
								<div class="clearfix">
									...
									echo ($course_students)?'<div class="col-md-3 col-sm-3 col-xs-6 nopad-all"><span class="course-list-students"><i class="sl-people"></i>'.$course_students .' '. esc_html__('Studesnts','deep').'</span></div>':'';
									echo '<div class="col-md-3 col-sm-3 col-xs-6 nopad-all"><span class="modern-viewers"><i class="sl-eyeglass"></i>'.deep_getViews(get_the_ID()) .' '. esc_html__('Viewers','deep').'</span></div>';
									?>
								</div>
							</div>
						</div>
