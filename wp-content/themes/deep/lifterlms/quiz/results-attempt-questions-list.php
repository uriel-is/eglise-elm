...
<?php foreach ( $attempt->get_question_objects() as $attempt_question ) :
	...
	<li class="llms-quiz-attempt-question type--<?php echo esc_attr( $quiz_question->get( 'question_type' ) ); ?> status--<?php echo esc_attr( $attempt_question->get_status() ); ?> <?php echo esc_attr( $attempt_question->is_correct() ) ? 'correct' : 'incorrect'; ?>"
		data-question-id="<?php echo esc_attr( $quiz_question->get( 'id' ) ); ?>"
		data-grading-manual="<?php echo esc_attr( $attempt_question->can_be_manually_graded() ) ? 'yes' : 'no'; ?>"
					...
					<?php if ( 'choices' === $quiz_question->get_auto_grade_type() ) : ?>
						<div class="llms-quiz-attempt-answer-section llms-correct-answer">
							...
							<?php foreach ( $quiz_question->get_correct_choice() as $aid ) :
								...
								<?php echo wp_kses_post( $choice->get_choice() ); ?>
							<?php endforeach; ?>
						</div>
					<?php elseif ( 'conditional' === $quiz_question->get_auto_grade_type() ) : ?>
						<div class="llms-quiz-attempt-answer-section llms-correct-answer">
							<p class="llms-quiz-results-label correct-answer"><?php _e( 'Correct answer: ', 'lifterlms' ); ?></p>
							<?php echo wp_kses_post( $quiz_question->get( 'correct_value' ) ); ?>
						</div>
					<?php endif; ?>
				<?php endif; ?>
