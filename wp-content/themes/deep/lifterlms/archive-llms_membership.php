			...
			<h2 style="">
				<?php
					...
					if ( get_query_var( 'paged' ) ) {
						...
						echo esc_html__('All Memberships','deep');
					}
				?>
			</h2>
	</div>
...
<hr class="vertical-space2">
	...
	<?php //do_action( 'lifterlms_sidebar' ); ?>
		...
		<div class="widget widget_michigan_course_categories">
			<h3> <?php esc_html_e('FILTER CATEGORIES','deep'); ?> </h3>
			<?php
			$m_cat_args = array(
			'type'			=> 'post',
			'child_of'		=> 0,
			...
			if ( ! empty( $m_categories ) && ! is_wp_error( $m_categories ) ){
				echo '<ul class="course-categories">';
					echo '<li class="course-category">
							<a class="hcolorf" href="' . esc_url( get_site_url() ) . '/memberships/" title="' . esc_attr( 'View all Categories', 'deep' ). '"> <i class="fa fa-group"></i> ' . esc_html__('All Memberships','deep'). '</a>
						</li>';
				foreach ( $m_categories as $m_category ) :
					...
					echo '
						<li class="course-category">
							<a class="hcolorf" href="' . esc_url( get_category_link( $m_category ) ) . '" title="' . esc_attr( sprintf( esc_html__( 'View all courses under %s', 'deep' ), $m_category->name ) ) . '">'. $cat_icon . esc_html( $m_category->name ) .'<span> (' . $m_category->count . ')</span>' . '</a>
						</li>';
				endforeach;
			echo '</ul>';
			}
						...
						elseif ( ! llms_is_user_member( get_current_user_id(), get_the_ID() ) ) {
							if ($single_price > 0 || $rec_price > 0) {
								...
								<form action="" method="post" id="hiddenform">
									...
									<input type="hidden" name="product_price" value="<?php echo esc_attr( $product->get_price() ); ?>" />
									<input type="hidden" name="product_sku" value="<?php echo esc_attr( $product->get_sku() ); ?>" />
									<input type="hidden" name="product_title" value="<?php echo esc_attr( $post->post_title ); ?>" />
							...
							global $post;
								if (get_the_terms($post->ID, 'membership_cat' )) {
									...
									foreach ( $categories as $category ) :
										...
										$typeName[] = '<a class="hcolorf" href="' . esc_url( get_category_link( $category ) ) . '" title="' . esc_attr( sprintf( esc_html__( 'View all memberships under %s', 'deep' ), $category->name ) ) . '">'. $cat_icon . esc_html( $category->name ). '</a>';
								...
								<?php foreach ( $llms_product as $option ):
									...
									<?php }elseif ( $option == 'recurring' ){
											foreach ( $llms_product->get_subscriptions() as $id => $sub ) :
												...
												<span class="llms-price-option-separator"><?php echo apply_filters( 'lifterlms_price_option_separator', esc_html__( 'or', 'deep' ), $llms_product ); ?></span>
											<?php } ?>
											<h4 class="llms-price"><span><?php echo wp_kses_post( $llms_product->get_subscription_price_html( $sub ) ); ?></span></h4>
										<?php endforeach;
										}
