...
$lesson_features = $deep_options['michigan_webnus_lesson_features'];
global $quiz;
...
?>
<?php
	if($deep_options['michigan_webnus_enable_breadcrumbs'] ) {
		//Breadcrumb
	...
	<div class="col-md-6">
		<div class="post-thumbnail quiz-thumbnail">
			<?php if(isset($lesson_features['image']) && $lesson_features['image'] && has_post_thumbnail()){
				...
				$no_image_src = ($deep_options['michigan_webnus_course_no_image_src']['url'])?$deep_options['michigan_webnus_course_no_image_src']['url']: get_template_directory_uri().'/images/course_no_image.png';
	...
	<div class="col-md-6">
		...
		<div class="llms-template-wrapper">
			<h4 class="llms-content-block">
				<?php printf( __( '<i class="fa-asterisk"></i> Attempts Remaining: <span class="llms-content llms-attempts">%s</span>', 'deep' ), $attempts_left ); ?>
			</h4>
		</div>
		<?php } //Time Limit
		if ( $time_limit ) { ?>
		...
		<div class="llms-template-wrapper">
			<h4 class="llms-content-block">
				<?php printf( __( '<i class="fa-clock-o"></i> Time Limit: <span class="llms-content">%s</span>', 'deep' ), LLMS_Date::convert_to_hours_minutes_string( $time_limit ) ); ?>
			</h4>
		</div>
		<?php } ?>
	</div>
